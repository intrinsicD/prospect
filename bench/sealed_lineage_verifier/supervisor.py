"""Host-bound cgroup-v2 supervisor for the LCV-001 lexical runtime.
...
SCHEMA_VERSION: Final = "lcv001-systemd-cgroup-supervisor-v1"
...
UNIT_PREFIX: Final = "lcv001-custody"
RUNTIME_MAX_SECONDS: Final = 180.0
_CLEANUP_RESERVE_SECONDS: Final = 6.0
_CAPTURE_LIMIT_BYTES: Final = 8 * 1024 * 1024
_UNIT_ENV: Final = "LCV001_CGROUP_UNIT"
_ROLE_ENV: Final = "LCV001_CGROUP_ROLE"
...
_GUARD_SOURCE: Final = (
    ...
    "assert os.environ['LCV001_CGROUP_UNIT']==unit;"
    "assert os.environ['LCV001_CGROUP_ROLE']==role;"
...
def _validate_role(role: str) -> None:
    if _ROLE_PATTERN.fullmatch(role) is None:
        raise SupervisorError("LCV-001 cgroup role grammar differs")
...
def _validate_unit_name(unit: str, role: str) -> None:
    ...
    if not unit.startswith(prefix) or not unit.endswith(".service"):
        raise SupervisorError("LCV-001 cgroup unit identity differs")
    identity = unit[len(prefix) : -len(".service")].split("-")
    if len(identity) != 3 or not all(value.isdecimal() and value for value in identity):
        raise SupervisorError("LCV-001 cgroup unit identity grammar differs")
...
def assert_current_cgroup(role: str) -> str:
    ...
    if observed_role != role:
        raise SupervisorError("LCV-001 cgroup role environment differs")
    _validate_unit_name(unit, role)
    try:
        lines = Path("/proc/self/cgroup").read_text(encoding="ascii").splitlines()
    except (OSError, UnicodeError) as error:
        raise SupervisorError("current LCV-001 cgroup identity is unavailable") from error
    for line in lines:
        fields = line.split(":", 2)
        if len(fields) == 3 and fields[:2] == ["0", ""] and Path(fields[2].rstrip("/")).name == unit:
            return unit
    raise SupervisorError(f"{role} lifecycle is outside its frozen LCV-001 cgroup")
...
def _validate_environment(environment: Mapping[str, str]) -> dict[str, str]:
    ...
    for name, value in environment.items():
        if (
            ...
            raise SupervisorError("LCV-001 frozen environment grammar differs")
        output[name] = value
    if not output:
        raise SupervisorError("LCV-001 frozen environment is empty")
    return output


def _validate_command(command: Sequence[str]) -> tuple[str, ...]:
    if not command or any(not isinstance(item, str) or not item or "\x00" in item for item in command):
        raise SupervisorError("LCV-001 command grammar differs")
    required_prefix = (str(LEXICAL_PYTHON), "-I", "-S", "-B")
    if len(command) < len(required_prefix) or tuple(command[:4]) != required_prefix:
        raise SupervisorError("LCV-001 command must retain the pinned lexical argv[0] and -I -S -B prefix")
    try:
        resolved = Path(command[0]).resolve(strict=True)
    except OSError as error:
        raise SupervisorError("LCV-001 lexical interpreter is unavailable") from error
    if resolved != REAL_PYTHON:
        raise SupervisorError("LCV-001 lexical interpreter terminal path differs")
    if _file_record(resolved) != _EXECUTABLE_PINS[REAL_PYTHON]:
        raise SupervisorError("LCV-001 terminal interpreter hash differs")
    return tuple(command)


def _validate_cwd(cwd: Path) -> Path:
    if not isinstance(cwd, Path) or not cwd.is_absolute():
        raise SupervisorError("LCV-001 working directory must be absolute")
    try:
        metadata = cwd.lstat()
        resolved = cwd.resolve(strict=True)
    except (FileNotFoundError, OSError) as error:
        raise SupervisorError("LCV-001 working directory is unavailable") from error
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode) or resolved != cwd:
        raise SupervisorError("LCV-001 working directory must be one exact real directory")
    return cwd
...
def _kill_process_group(process: subprocess.Popen[bytes], *, deadline: float) -> None:
    ...
    try:
        ...
        raise SupervisorError("LCV-001 local supervisor process group could not be reaped") from error
    if _process_group_exists(process.pid):
        raise SupervisorError("LCV-001 local supervisor left a live process group")
...
def _read_capture(stream: object, label: str) -> str:
    if not hasattr(stream, "fileno") or not hasattr(stream, "seek") or not hasattr(stream, "read"):
        raise SupervisorError(f"LCV-001 {label} capture stream differs")
    size = os.fstat(stream.fileno()).st_size
    if size > _CAPTURE_LIMIT_BYTES:
        raise SupervisorError(f"LCV-001 {label} exceeded its capture bound")
    stream.seek(0)
    payload = stream.read()
    if not isinstance(payload, bytes):
        raise SupervisorError(f"LCV-001 {label} capture was not binary")
...
def _enforce_capture_bound(stdout_stream: object, stderr_stream: object) -> None:
    if not hasattr(stdout_stream, "fileno") or not hasattr(stderr_stream, "fileno"):
        raise SupervisorError("LCV-001 capture streams differ")
    ...
    if stdout_bytes > _CAPTURE_LIMIT_BYTES or stderr_bytes > _CAPTURE_LIMIT_BYTES:
        raise SupervisorError("LCV-001 child exceeded its capture bound")
    if stdout_bytes + stderr_bytes > _CAPTURE_LIMIT_BYTES:
        raise SupervisorError("LCV-001 child exceeded its aggregate capture bound")
        ...
        try:
            while process.poll() is None:
                ...
                if cancel_event is not None and cancel_event.is_set():
                    raise SupervisorError("LCV-001 supervisor was cancelled")
            ...
            _enforce_capture_bound(stdout_stream, stderr_stream)
            if _process_group_exists(process.pid):
                raise SupervisorError("LCV-001 local supervisor left a live descendant")
...
def _cleanup_unit(unit: str, *, deadline: float) -> dict[str, object]:
    ...
    try:
        ...
        raise SupervisorError(f"could not verify inactive LCV-001 cgroup unit: {unit}") from error
    state_name = state.stdout.strip()
    if state.returncode == 0 or state_name not in {"failed", "inactive", "unknown"}:
        raise SupervisorError(f"LCV-001 cgroup unit remained active: {unit} ({state_name})")
    cgroup_paths = _unit_cgroup_paths(unit)
    if cgroup_paths:
        raise SupervisorError(f"LCV-001 cgroup path remained after cleanup: {unit}")
    return {
        "actions": actions,
...
def validate_cleanup_receipt(value: object, *, role: str) -> dict[str, object]:
    ...
    if not isinstance(value, dict) or set(value) != {
        ...
        raise SupervisorError("LCV-001 cleanup receipt schema differs")
    unit = value.get("unit")
    if not isinstance(unit, str):
        raise SupervisorError("LCV-001 cleanup receipt unit differs")
    _validate_unit_name(unit, role)
    if (
        value.get("schema_version") != "lcv001-cgroup-cleanup-v1"
        ...
        raise SupervisorError("LCV-001 cleanup receipt postcondition differs")
    actions = value.get("actions")
    if not isinstance(actions, list) or len(actions) != 3:
        raise SupervisorError("LCV-001 cleanup receipt action count differs")
    for expected_action, action in zip(("kill", "stop", "reset-failed"), actions, strict=True):
        if not isinstance(action, dict) or action.get("action") != expected_action:
            raise SupervisorError("LCV-001 cleanup receipt action order differs")
        if set(action) == {"action", "error"}:
            if not isinstance(action["error"], str) or not action["error"]:
                raise SupervisorError("LCV-001 cleanup error receipt differs")
            ...
            if (
                ...
                raise SupervisorError("LCV-001 cleanup command receipt differs")
        else:
            raise SupervisorError("LCV-001 cleanup action schema differs")
    return value
...
def run(
    ...
    *,
    ...
    ``timeout_seconds`` is an outer total-wall ceiling and may only tighten the
    frozen 180-second systemd ceiling.  A small portion is reserved so timeout
    and interruption paths can still kill and verify the unit before returning.
    """

    if (
        not isinstance(timeout_seconds, (int, float))
        or not math.isfinite(float(timeout_seconds))
        or not _CLEANUP_RESERVE_SECONDS < float(timeout_seconds) <= RUNTIME_MAX_SECONDS
    ):
        raise SupervisorError("LCV-001 total-wall timeout boundary differs")
    ...
    service_deadline = deadline - _CLEANUP_RESERVE_SECONDS
    try:
        ...
        receipt = _cleanup_unit(unit, deadline=deadline)
        try:
            error.__dict__["lcv001_cleanup_receipt"] = receipt
...
def preflight(environment: Mapping[str, str]) -> dict[str, object]:
    ...
    source = (
        "import json,os,sys;"
        "unit=os.environ['LCV001_CGROUP_UNIT'];"
        "lines=open('/proc/self/cgroup',encoding='ascii').read().splitlines();"
        "assert any(line.split(':',2)[:2]==['0',''] and "
        "line.split(':',2)[2].rstrip('/').endswith('/'+unit) for line in lines);"
    ...
    if completed.returncode != 0 or completed.stderr:
        raise SupervisorError(f"LCV-001 cgroup preflight failed: {completed.stderr[-1000:]}")
    import json

    try:
        value = json.loads(completed.stdout)
    except json.JSONDecodeError as error:
        raise SupervisorError("LCV-001 cgroup preflight emitted invalid JSON") from error
    if (
        ...
        raise SupervisorError("LCV-001 cgroup preflight runtime identity differs")
