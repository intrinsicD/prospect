"""One-row, source-only MM-011 prediction worker.
...
from bench.multimodal_causal_assay import predictor, preparation, records
...
SCHEMA_VERSION: Final = "mm011-source-worker-v1"
...
def _read_config(path: str | Path) -> dict[str, object]:
    ...
    if set(value) != expected or value.get("schema_version") != "mm011-worker-config-v1":
    ...
    *,
    ...
    if arm == "appearance":
        ...
        fit_parameters, fit_gains, fit_biases, history_hash = _validated_global_fit_record(
            record["history_fit"],
            arm=arm,
            context_key=f"mm011/source/history/full/{arm}",
            source=source,
            target=target,
            fit_mask=geometry.FULL_MASK,
            output_mask=geometry.FULL_MASK,
        ...
        for output_parity, fit_record in enumerate(parity_fits):
            ...
            if arm == "appearance":
                ...
                _validated_global_fit_record(
                    fit_record,
                    arm=arm,
                    context_key=f"mm011/source/history/checkerboard/output-{output_parity}/{arm}",
                    source=previous,
                    target=current,
                    fit_mask=fit_mask,
                    output_mask=output_mask,
...
def validate_worker_output(
    ...
    *,
    ...
    for index, role in enumerate(PREDICTION_ROLES):
        ...
        if item.get("mm011_sha256") != records.scientific_array_sha256(
            f"prediction:{role}", predictions[index], protocol_sha256=protocol_sha256
