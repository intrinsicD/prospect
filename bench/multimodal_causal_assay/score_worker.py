"""One-row, fitting-free target scorer for frozen MM-011 predictions.
...
from bench.multimodal_causal_assay import preparation, records, scoring

SCHEMA_VERSION: Final = "mm011-score-worker-v1"
SCORE_SCHEMA_VERSION: Final = "mm011-row-score-v1"
SUPERVISOR_SCHEMA_VERSION: Final = "mm011-supervisor-prediction-commit-v1"
WORKER_SCHEMA_VERSION: Final = "mm011-source-worker-v1"
WORKER_CONFIG_SCHEMA_VERSION: Final = "mm011-worker-config-v1"
...
def _validate_worker_evidence(
    ...
    *,
    ...
    for index, role in enumerate(PREDICTION_ROLES):
        ...
        if type(item) is not dict or set(item) != {"mm011_sha256", "predictor_sha256"}:
        ...
        if item.get("mm011_sha256") != expected:
            raise ScoreWorkerError(f"worker evidence MM-011 prediction hash differs for {role}")
