"""Canonical, target-agnostic record primitives for MM-009.
...
SCHEMA_VERSION: Final = "mm009-records-v1"
PROTOCOL_PATH: Final = Path("docs/research/2026-07-16-mm009-causal-deformation-appearance-prediction-protocol.md")
# Frozen after the independent implementation/custody audit and before any real
# MM-007 scientific input was opened by MM-009.
PROTOCOL_SHA256: Final[str] = "ca39f7cea6a2a5b041956b419bf3530dd54eb8403096963a044d7fcf1e2121cc"

MM009_ARRAY_TAG: Final = b"MM009-scientific-array-v1\0"
MM009_JSON_TAG: Final = b"MM009-canonical-json-v1\0"
...
PARENT_PINS: Final[Mapping[str, ParentPin]] = MappingProxyType(
    {
        "artifact-manifest.json": ParentPin("db0b6654ab098dc9a3ec93e4a6de8820bbe5860d44974645e9a5ee7dad1537fb", 0o644),
        "input-manifest.json": ParentPin("1f83c805e6c5d75f4f1d5a2102d471c15bbc6bb787960cb5ae630bd2260faa1f", 0o644),
        "formal-start.json": ParentPin("ea5c7bda870d71ead3172c1fc6e504d6a6b02d2ba785e9fd2fc75a91c667eee3", 0o444),
        "MM-007-evidence.json": ParentPin("13dfa89e541e6122263ea9814d42fb328da303dcc74556cdaaa5d5860d99abaf", 0o644),
        "MM-007-results.json": ParentPin("3c92729e1e5c18c14461e36602bdb86acd31750d9f5a85f535cd33a43fb9c47b", 0o644),
        "MM-007-report.md": ParentPin("b18760128941ab2eff893b8c0afc469b92f71077d489e060d56519407990b8a2", 0o644),
        "MM-007-protocol.md": ParentPin("24bbac1855cc2b51d2a65012b9c63037637c53555b86bbad7c66a6249108a73c", 0o644),
        "MM-007-frames-64x64.npz": ParentPin("fbc79d81a06720175139f7106745bd58f8788f43cc5a2fcd10658d186909797f", 0o644),
    }
)

# MM-007 retains its lineage inputs beside the sealed result files.  They are not
# scientific authority for MM-009: preparation checks only that this exact top-level
# name is a real directory, then neither traverses nor copies it.
...
def canonical_json_bytes(value: JsonValue) -> bytes:
    """Return the unique ASCII JSON encoding used by MM-009 (without LF)."""
    ...
    try:
        ...
        raise RecordValidationError("value is not canonical MM-009 JSON") from error
    return payload
...
def _canonical_array(value: np.ndarray) -> np.ndarray:
    ...
    if value.dtype.hasobject or value.dtype.fields is not None or value.dtype.subdtype is not None:
        raise RecordValidationError("scientific array dtype is outside the closed MM-009 grammar")
...
def _digest_opened_file(path: str | Path) -> tuple[str, os.stat_result]:
    ...
    if not hasattr(os, "O_NOFOLLOW"):
        raise RuntimeError("MM-009 requires O_NOFOLLOW")
