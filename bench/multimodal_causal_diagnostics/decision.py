"""Pure eight-video gate arithmetic for frozen MM-009 scoring primitives.
...
from bench.multimodal_causal_diagnostics import scoring

PROTOCOL_SHA256: Final[str] = "ca39f7cea6a2a5b041956b419bf3530dd54eb8403096963a044d7fcf1e2121cc"
SCHEMA_VERSION: Final = "mm009-decision-v1"

if scoring.PROTOCOL_SHA256 != PROTOCOL_SHA256:
    raise RuntimeError("MM-009 decision and scoring bind different protocol bytes")
...
class DecisionError(ValueError):
    """Raised when MM-009 decision evidence violates its strict schema."""
...
def derive_decision(evidence: DecisionEvidence) -> DecisionSummary:
    """Recompute all MM-009 family gates and apply the frozen precedence ladder."""
    ...
    if prerequisite_failures or incomplete:
        ...
        passing = ()
        inconclusive = ()
        decision: DecisionLabel = "invalid_MM009"
    elif invalid_nulls:
        invalid_reasons = invalid_nulls
        passing = ()
        inconclusive = ()
        decision = "invalid_MM009_real_negative_control"
    elif inconclusive:
        passing = ()
        decision = "MM009_inconclusive"
    elif passing:
...
def validate_decision(evidence: DecisionEvidence, claimed: DecisionSummary) -> None:
    ...
    if type(claimed) is not DecisionSummary or not _deep_equal(derive_decision(evidence), claimed):
        raise DecisionError("claimed MM-009 decision differs from pure recomputation")
