"""Future-blind source-side prediction primitives for MM-009.
...
SCHEMA_VERSION: Final = "mm009-source-predictor-v1"
...
_ARRAY_SCOPE_SHA256: Final = hashlib.sha256(
    b"MM009-source-side-scientific-array-v1\0"
).hexdigest()
_FULL_CONTEXTS: Final[dict[Literal["affine", "combined"], str]] = {
    "affine": "mm009/source/history/full/affine",
    "combined": "mm009/source/history/full/combined",
}
...
def array_sha256(value: np.ndarray, *, role: str) -> str:
    ...
    This deliberately uses an MM-009-specific scope layered over the frozen v2.2
...
def fit_bias_only_control(
    ...
    *,
    ...
    future control receives a fresh MM-009 forecast hash.
    """
...
def _checker_context(
    ...
    return f"mm009/source/history/checkerboard/output-{output_parity}/{arm}"
