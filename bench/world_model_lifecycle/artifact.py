...
FORMAL_LAUNCH_MARKER_NAME = "formal-launch-v1.7.0.json"
...
def formal_launch_marker_path(
    ...
    *,
    ...
    if (
        ...
        or binding["protocol"].get("version") != "1.7.0"
    ):
        raise ValueError("formal binding is not a WM-001 protocol-1.7 binding")
    ...
    if output.parent != binding_root or not output.name or output.name in {".", ".."}:
        raise ValueError("formal output must be one new direct child of results/formal/<binding-sha256>")
...
def _claim_formal_launch(
    ...
    *,
    ...
    """Publish the durable producer record as protocol 1.7's sole formal claim."""
    ...
    if os.path.lexists(marker_path):
        raise RuntimeError("protocol 1.7 already has a formal launch claim; same-version resume or retry is forbidden")
    ...
    try:
        ...
        raise RuntimeError(
            "protocol 1.7 already has a formal launch claim; same-version resume or retry is forbidden"
        ) from None
    ...
    if (
        ...
        raise RuntimeError("protocol 1.7 launch publication did not preserve one exact inode")
