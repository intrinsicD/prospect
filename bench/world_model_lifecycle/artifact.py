...
FORMAL_LAUNCH_MARKER_NAME = "formal-launch-v1.19.0.json"
FORMAL_CONFIRMATION_NAME = "confirmation-v1.19.0"
...
def formal_launch_marker_path(
    ...
    *,
    ...
    if (
        ...
        or binding["protocol"].get("version") != "1.19.0"
    ):
        raise ValueError("formal binding is not a WM-001 protocol-1.19 binding")
...
def _claim_formal_launch(
    ...
    *,
    ...
    """Publish the durable producer record as protocol 1.19's sole formal claim."""
    ...
    if os.path.lexists(marker_path):
        raise RuntimeError("protocol 1.19 already has a formal launch claim; same-version resume or retry is forbidden")
    ...
    try:
        with hold_accepted_binding_rehearsal(
            ...
            try:
                ...
                raise RuntimeError(
                    "protocol 1.19 already has a formal launch claim; same-version resume or retry is forbidden"
                ) from None
            ...
            if (
                ...
                raise RuntimeError(
                    "protocol 1.19 launch publication did not preserve one exact inode"
                )
