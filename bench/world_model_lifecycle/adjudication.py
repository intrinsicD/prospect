"""One-shot terminal adjudication for the sole formal WM-001 v1.10 attempt.
...
replay or failure-package action.  Once that claim exists, protocol 1.10 is
...
ADJUDICATION_RESULTS_ROOT = REPO / "bench" / "world_model_lifecycle" / "results" / "adjudication-v1.10"
FORMAL_ADJUDICATION_PACKAGE_PATH = ADJUDICATION_RESULTS_ROOT / "formal-adjudication-v1.10.0"
FORMAL_ADJUDICATION_CLAIM_MARKER = (
    REPO / "bench" / "world_model_lifecycle" / "results" / "formal" / "formal-adjudication-v1.10.0.json"
)
FORMAL_AUDIT_ATTEMPT_PATH = operator_module.FORMAL_AUDIT_ATTEMPT_PATH
FORMAL_SEMANTIC_REVIEW_PATH = REPO / "artifacts" / "wm001-reviews" / "formal-v1.10.0.json"
...
class AdjudicationError(ValueError):
    """Evidence cannot enter the sole terminal WM-001 v1.10 adjudication."""
...
def _verify_launch(
    *,
    ...
    if (
        ...
        or launch.get("protocol_version") != "1.10.0"
        ...
        raise AdjudicationError("formal result does not bind the unique v1.10 launch record")
    _manifested_digest(
        producer_manifest,
...
def _load_upstream(
    ...
    *,
    ...
    if (
        ...
        or audit_execution.get("outcome_support_files")
        != [
            "producer_bootstrap.py",
            "protocol.json",
            "schemas/raw-result.schema.json",
        ]
        or audit_execution.get("outcome_argv_role")
        != [
    ...
    if (
        ...
        or not isinstance(runtime_support, list)
        or [
            row.get("path") if isinstance(row, Mapping) else None
            for row in runtime_support
        ]
        != [
            "producer_bootstrap.py",
            "protocol.json",
        ...
        or not isinstance(runtime_support[0], Mapping)
        or runtime_support[0].get("bytes")
        != len(source_payloads["producer_bootstrap.py"])
...
def _load_audit_attempt(path: Path) -> _AuditAttempt:
    ...
    if (
        ...
        raise AdjudicationError("operator evidence is not the formal v1.10 audit attempt")
    names: list[str] = []
    for row in rows:
        if (
...
def verify_semantic_review_for_adjudication(
    audit_attempt: Path,
    semantic_review: Path,
    disposition: Disposition,
) -> dict[str, object]:
    """Strictly validate the canonical review without consuming adjudication."""

    if disposition not in {"accepted", "rejected"}:
        raise AdjudicationError(
            "semantic review disposition must be accepted or rejected"
        )
    attempt = _load_audit_attempt(audit_attempt)
    ...
    try:
        ...
        if _upstream_identity(reopened) != _upstream_identity(upstream):
            raise AdjudicationError(
                "formal producer changed during semantic-review verification"
            )
        return dict(review)
    finally:
...
def _prepare_output_paths() -> Path:
    ...
    if package.name != "formal-adjudication-v1.10.0" or marker.name != "formal-adjudication-v1.10.0.json":
        raise AdjudicationError("formal adjudication paths are not version-scoped to v1.10.0")
    ...
    if os.path.lexists(marker):
        raise _AdjudicationRetired("WM-001 protocol 1.10 adjudication claim is already consumed")
...
def _publish_adjudication_claim(
    *,
    ...
    try:
        ...
        raise _AdjudicationRetired("WM-001 protocol 1.10 adjudication claim is already consumed") from error
    except OSError as error:
...
def _verify_adjudication_package(
    ...
    *,
    ...
    if not allow_staging and package != FORMAL_ADJUDICATION_PACKAGE_PATH:
        raise AdjudicationError("public adjudication verification requires the canonical v1.10 package")
    if require_outer:
        try:
...
def verify_adjudication_package(path: Path) -> dict[str, object]:
    """Verify only an outer-finalized canonical v1.10 adjudication package."""
...
def _recover_adjudication_package(
    *,
    ...
    if (
        package.name != "formal-adjudication-v1.10.0"
        or marker_path.name != "formal-adjudication-v1.10.0.json"
    ):
            ...
            if completed is not None:
                if not allow_completed:
                    raise _AdjudicationRetired(
                        "WM-001 protocol 1.10 adjudication is already outer-finalized"
                    )
                return completed
...
def recover_adjudication_package() -> dict[str, object]:
    """Finalize a consumed v1.10 claim without another audit replay."""
...
def create_adjudication_package(
    *,
    ...
    """Consume v1.10 and publish one accepted/rejected terminal package."""
