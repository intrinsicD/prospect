"""One-shot terminal adjudication for the sole formal WM-001 v1.8 attempt.
...
replay or failure-package action.  Once that claim exists, protocol 1.8 is
...
ADJUDICATION_RESULTS_ROOT = REPO / "bench" / "world_model_lifecycle" / "results" / "adjudication-v1.8"
FORMAL_ADJUDICATION_PACKAGE_PATH = ADJUDICATION_RESULTS_ROOT / "formal-adjudication-v1.8.0"
FORMAL_ADJUDICATION_CLAIM_MARKER = (
    REPO / "bench" / "world_model_lifecycle" / "results" / "formal" / "formal-adjudication-v1.8.0.json"
)
FORMAL_AUDIT_ATTEMPT_PATH = operator_module.FORMAL_AUDIT_ATTEMPT_PATH
FORMAL_SEMANTIC_REVIEW_PATH = REPO / "artifacts" / "wm001-reviews" / "formal-v1.8.0.json"
...
class AdjudicationError(ValueError):
    """Evidence cannot enter the sole terminal WM-001 v1.8 adjudication."""
...
def _verify_launch(
    *,
    ...
    if (
        ...
        or launch.get("protocol_version") != "1.8.0"
        ...
        raise AdjudicationError("formal result does not bind the unique v1.8 launch record")
    _manifested_digest(
        producer_manifest,
...
def _load_upstream(
    ...
    *,
    ...
    if (
        ...
        or result.get("schema") != "prospect.world-model-lifecycle.raw-result.v8"
        or result.get("experiment_id") != "WM-001"
        or result.get("protocol_version") != "1.8.0"
        or result.get("lane") != "formal"
        or result.get("claim_eligible") is not True
        or result.get("formal_binding_sha256") != binding_sha256
        or binding.get("schema") != "prospect.world-model-lifecycle.formal-binding.v8"
        or binding.get("experiment_id") != "WM-001"
        or not isinstance(protocol, Mapping)
        or protocol.get("version") != "1.8.0"
...
def _load_audit_attempt(path: Path) -> _AuditAttempt:
    ...
    if (
        ...
        raise AdjudicationError("operator evidence is not the formal v1.8 audit attempt")
    names: list[str] = []
    for row in rows:
        if (
...
def _prepare_output_paths() -> Path:
    ...
    if package.name != "formal-adjudication-v1.8.0" or marker.name != "formal-adjudication-v1.8.0.json":
        raise AdjudicationError("formal adjudication paths are not version-scoped to v1.8.0")
    ...
    if os.path.lexists(marker):
        raise _AdjudicationRetired("WM-001 protocol 1.8 adjudication claim is already consumed")
...
def _publish_adjudication_claim(
    *,
    ...
    try:
        ...
        raise _AdjudicationRetired("WM-001 protocol 1.8 adjudication claim is already consumed") from error
    except OSError as error:
...
def _verify_adjudication_package(
    ...
    *,
    ...
    if not allow_staging and package != FORMAL_ADJUDICATION_PACKAGE_PATH:
        raise AdjudicationError("public adjudication verification requires the canonical v1.8 package")
    if require_outer:
        try:
...
def verify_adjudication_package(path: Path) -> dict[str, object]:
    """Verify only an outer-finalized canonical v1.8 adjudication package."""
...
def _recover_adjudication_package(
    *,
    ...
    if (
        package.name != "formal-adjudication-v1.8.0"
        or marker_path.name != "formal-adjudication-v1.8.0.json"
    ):
            ...
            if completed is not None:
                if not allow_completed:
                    raise _AdjudicationRetired(
                        "WM-001 protocol 1.8 adjudication is already outer-finalized"
                    )
                return completed
...
def recover_adjudication_package() -> dict[str, object]:
    """Finalize a consumed v1.8 claim without another audit replay."""
...
def create_adjudication_package(
    *,
    ...
    """Consume v1.8 and publish one accepted/rejected terminal package."""
