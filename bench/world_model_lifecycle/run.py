...
def _development_lifecycle_paths() -> _DevelopmentLifecyclePaths:
    ...
    return _DevelopmentLifecyclePaths(
        results_root=root,
        qualification=root / "qualification-v1.20.0",
        closure=root / "development-closure-v1.20.0.json",
        diagnostics=root / "diagnostics-v1.20.0",
    )
...
def main() -> int:
    ...
    if arguments.lane == "formal":
        ...
        if os.path.lexists(existing_launch_marker):
            print(
                "WM-001 protocol 1.20 formal launch already consumed; same-version retry is forbidden",
                file=sys.stderr,
            )
            return 1
    else:
        ...
        if os.path.lexists(development_paths.qualification):
            print(
                "WM-001 protocol 1.20 development qualification already consumed; "
        ...
        if os.path.lexists(development_paths.closure):
            print(
                "WM-001 protocol 1.20 development is closed; additional same-version rehearsals are forbidden",
                file=sys.stderr,
            )
            return 1
