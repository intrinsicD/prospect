"""Atomic custody-preserving operator entry points for WM-001 protocol 1.17.
...
DEVELOPMENT_QUALIFICATION_PATH = DEVELOPMENT_RESULTS_ROOT / "qualification-v1.17.0"
OPERATOR_RESULTS_ROOT = REPO / "bench" / "world_model_lifecycle" / "results" / "operator-v1.17"
...
OUTER_COMPLETIONS_ROOT = REPO / "bench" / "world_model_lifecycle" / "results" / "outer-completions" / "v1.17"
FORMAL_BINDING_ATTEMPT_PATH = BINDING_ATTEMPTS_ROOT / "formal-binding-v1.17.0"
DEVELOPMENT_AUDIT_ATTEMPT_PATH = AUDIT_ATTEMPTS_ROOT / "development-audit-v1.17.0"
FORMAL_AUDIT_ATTEMPT_PATH = AUDIT_ATTEMPTS_ROOT / "formal-audit-v1.17.0"
FORMAL_AUDIT_CLAIM_MARKER = (
    ...
    / "results"
    / "formal"
    / "formal-audit-v1.17.0.json"
)
CLOSURE_ATTEMPT_PATH = CLOSURE_ATTEMPTS_ROOT / "development-closure-v1.17.0"
...
class _FormalAuditRetired(OperatorError):
    """The sole protocol-1.17 formal audit claim has already been consumed."""
...
def _verify_published_operator_attempt(
    ...
    *,
    ...
    if root != expected_attempt:
        raise OperatorError("published operator attempt is not its canonical protocol-1.17 path")
...
def inspect_unfinalized_operator_attempt(path: Path) -> dict[str, object]:
    ...
    if root != expected_attempt:
        raise OperatorError("unfinalized operator attempt is not its canonical protocol-1.17 path")
...
def _binding_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create one atomic WM-001 v1.17 binding attempt",
        allow_abbrev=False,
    )
...
def _publish_formal_audit_claim(
    *,
    ...
    if os.path.lexists(marker):
        raise _FormalAuditRetired("WM-001 protocol 1.17 formal audit claim is already consumed")
    ...
    try:
        ...
        try:
            ...
            raise _FormalAuditRetired("WM-001 protocol 1.17 formal audit claim is already consumed") from error
        except OSError as error:
...
def binding_main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments.output != FORMAL_BINDING_ATTEMPT_PATH:
        raise OperatorError(
            f"formal binding output must be the canonical protocol-1.17 attempt path {FORMAL_BINDING_ATTEMPT_PATH}"
        )
    ...
    if lane == "development":
        if arguments.output != DEVELOPMENT_AUDIT_ATTEMPT_PATH:
            raise OperatorError(
                ...
                f"protocol-1.17 attempt path {DEVELOPMENT_AUDIT_ATTEMPT_PATH}"
            )
        ...
        if os.path.lexists(binding_module.DEVELOPMENT_CLOSURE_PATH):
            raise OperatorError("WM-001 protocol 1.17 development audit is retired after development closure")
    else:
        if arguments.output != FORMAL_AUDIT_ATTEMPT_PATH:
            raise OperatorError(
                f"formal audit output must be the sole canonical protocol-1.17 attempt path {FORMAL_AUDIT_ATTEMPT_PATH}"
            )
...
def closure_main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments.output != CLOSURE_ATTEMPT_PATH:
        raise OperatorError(
            f"development closure output must be the canonical protocol-1.17 attempt path {CLOSURE_ATTEMPT_PATH}"
        )
    if arguments.producer != DEVELOPMENT_QUALIFICATION_PATH:
        raise OperatorError(
    ...
    if os.path.lexists(marker):
        raise OperatorError(
            "WM-001 protocol 1.17 development closure marker already "
    ...
    if arguments.audit_attempt != DEVELOPMENT_AUDIT_ATTEMPT_PATH:
        raise OperatorError(
            f"development closure requires the canonical protocol-1.17 audit attempt {DEVELOPMENT_AUDIT_ATTEMPT_PATH}"
        )
