...
accepted only at the exact protocol budgets; the v1.7 development rehearsal
...
def _runtime_custody_value(payload: bytes) -> tuple[dict[str, object], int]:
    ...
    if schema == "prospect.world-model-lifecycle.formal-binding.v7":
...
def _verify_live_bootstrap_custody() -> dict[str, object]:
    ...
    elif seal.get("schema") == "prospect.world-model-lifecycle.formal-binding.v7":
...
def run_restart_parity(
    *,
    ...
    closure_block = (
        ...
        if runtime_seal.get("schema") == "prospect.world-model-lifecycle.formal-binding.v7"
        else runtime_seal
    )
