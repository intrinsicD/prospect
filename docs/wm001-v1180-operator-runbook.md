# WM-001 v1.18.0 operator runbook

Status: prospective. Seal this runbook with the exact v1.18 protocol,
implementation, schemas, lock, tests, and independent review before any v1.18
producer path is created.

Protocols 1.10.0 through 1.17.0 are terminally retired. Version 1.17 passed
every prospective, development, audit, closure, preformal, binding, preflight,
and final stop/go gate. Its binding-keyed result-free rehearsal returned zero
and was authentically accepted and outer-finalized with no formal path. The
mandatory separate verifier then rejected its valid matrix identity because it
read a member forbidden by formal-binding v10. The v1.18 verifier instead uses
the sealed protocol identity; every fixture omits the forbidden member, and the
decisive integration test requires the exact launcher-stored bytes to pass the
independent verifier.

Do not inspect or use any retained K or performance value. Nothing in this
runbook repairs, resumes, removes, or reuses v1.10 through v1.17 evidence,
environments, wheels, seals, locks, bindings, outer completions, or diagnostics.
The exact disposition is documented in the
[v1.17 verifier failure](wm001-v1170-independent-rehearsal-verifier-failure.md).

The active protocol must directly supersede v1.17.0 and pin its exact sealed
protocol SHA-256 as
...
QA_PY="/home/alex/.venvs/prospect-wm001-v118-reviewed/bin/python"
RUNTIME_PY="/home/alex/.venvs/prospect-wm001-v118-reviewed-runtime/bin/python"
...
DEV="$DEV_ROOT/qualification-v1.18.0"
DEV_DIAGNOSTICS="$DEV_ROOT/diagnostics-v1.18.0"
RUNTIME_SEAL="$DEV_ROOT/runtime-seal-v1.18.0.json"
DEV_CLOSURE="$DEV_ROOT/development-closure-v1.18.0.json"
PREFORMAL_ROOT="$DEV_ROOT/v1.18.0/preformal"
PREFORMAL_CLAIM="$DEV_ROOT/v1.18.0/.preformal.staging"
PREFORMAL="$PREFORMAL_ROOT/preformal-test-report-v1.18.0.json"

OPERATOR_ROOT="$REPO/bench/world_model_lifecycle/results/operator-v1.18"
DEV_AUDIT="$OPERATOR_ROOT/audits/development-audit-v1.18.0"
CLOSURE_ATTEMPT="$OPERATOR_ROOT/closures/development-closure-v1.18.0"
BINDING_ATTEMPT="$OPERATOR_ROOT/bindings/formal-binding-v1.18.0"
BINDING="$BINDING_ATTEMPT/formal-binding.json"
FORMAL_INPUT_PREFLIGHT="$BINDING_ATTEMPT/formal-input-preflight.json"
REHEARSAL_ATTEMPT="$OPERATOR_ROOT/rehearsals/accepted-binding-rehearsal-v1.18.0"
REHEARSAL_CLAIM_ROOT="$REPO/bench/world_model_lifecycle/results/rehearsals/v1.18"

OUTER_ROOT="$REPO/bench/world_model_lifecycle/results/outer-completions/v1.18"
RUNTIME_LOCK="$REPO/bench/world_model_lifecycle/results/.wm001-v1.18-runtime.lock"
FORMAL_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-launch-v1.18.0.json"
FORMAL_AUDIT="$OPERATOR_ROOT/audits/formal-audit-v1.18.0"
FORMAL_AUDIT_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-audit-v1.18.0.json"
ADJUDICATION_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-adjudication-v1.18.0.json"
ADJUDICATION_ROOT="$REPO/bench/world_model_lifecycle/results/adjudication-v1.18"
ADJUDICATION_PACKAGE="$ADJUDICATION_ROOT/formal-adjudication-v1.18.0"

REVIEW="$REPO/docs/wm001-v1180-prospective-harness-review.json"
SEMANTIC_REVIEW="$REPO/artifacts/wm001-reviews/formal-v1.18.0.json"
...
require_v118_lifecycle_absent() {
  ...
  if find "$REPO/bench/world_model_lifecycle/results/formal" \
      -mindepth 1 -name '*v1.18.0*' -print -quit 2>/dev/null |
      grep -q .
  then
    echo "unexpected v1.18 formal lifecycle path exists" >&2
    return 2
  fi
}
```

The development masters are exactly `1787261725, 1697528199`. The formal
masters are exactly `952286440, 4273641788, 1748047518, 2531734648,
2611012043, 1851041586, 1135019273, 1670867274`. The verifier must verify 170
unique prior masters and 23,120 unique prior streams through v1.17 before any
...
WHEELHOUSE="$(mktemp -d /tmp/prospect-wm001-v118-wheelhouse.XXXXXX)"
...
generated WM-001 header must name v1.18.0. The Prospect row must equal the
...
lines = [
    "# WM-001 protocol 1.18.0 isolated live execution closure.",
    "# Versions preserve the v1.4 scientific runtime; each v1.18 digest covers",
...
```bash
REVIEW_DRAFT="$REPO/docs/wm001-v1180-prospective-harness-review.draft.json"
test -f "$REVIEW_DRAFT"
...
- the canonical verifier accepts only the original v1.18 development bundle
...
- the preserved verifier rejects a missing or changed referenced stream and
  any additional report or `preformal-v1.18.0-command-*` member; and
...
- direct supersession of v1.17.0 protocol SHA-256
  `b915d70eef0b09c7562b04f7c9f2e416cd12249c1b512108e11759d008473905`;
- scientific-block SHA-256
  `fa44fd93a672db3905d45a0e99c568985e7e2e5d02d32043c830db413005a5c3`;
- 10 current masters, 1,360 current streams, 170 prior masters, and 23,120
...
- unchanged raw-result v9 and formal-binding v10 representation identities,
  with only the v1.18 protocol and formal-seed constants updated; and
...
```bash
COMMAND10_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v118-command10.XXXXXX)"
...
if [ "$COMMAND10_RC" -ne 0 ] || [ -s "$COMMAND10_STDERR" ]; then
  echo "result-free command 10 failed or wrote stderr; v1.18 is retired" >&2
...
stderr retires v1.18; do not repair its source, wheel, environment, seal, or
capture in place.
...
v1.18 producer root. Verify the exact real-subprocess branch tests explicitly
...
nonzero return, or nonempty stderr consumes and retires v1.18; do not repeat
...
the v1.18 development qualification even if any later phase fails.
...
or missing ordinary audit retires v1.18; never rerun it.
...
binding, or stop/go failure retires v1.18. Absence of a downstream path never
...
`$DEV_ROOT/v1.18.0/.preformal.staging`, then fsyncs its parent before its first
...
require_absent "$DEV_ROOT/v1.18.0/.preformal.staging"
...
either path or retry v1.18. A successful generation is not sufficient by
...
version `1.18.0`, `passed: true`, and digests for the binding, preformal report,
development closure, accepted-closure evidence, runtime conformance, and
auditor source. Any missing, false, stale, or mismatched receipt retires v1.18.
...
FORMAL="$FORMAL_BINDING_ROOT/confirmation-v1.18.0"
...
REHEARSAL_OUTER_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v118-rehearsal-operator.XXXXXX)"
...
then
  REHEARSAL_RECOVERY_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v118-rehearsal-recovery.XXXXXX)"
  set +e
  "${SAFE_ENV[@]}" "$RUNTIME_PY" -I -S -B "$LAUNCH" \
    --bootstrap "$BOOTSTRAP" \
    --rehearse-accepted-binding "$BINDING" \
...
unfinalized evidence—or any created formal path—retires v1.18. Recovery can
...
# and every alternate/current v1.18 formal, audit, review, and adjudication
...
sibling left by an earlier launcher, contaminates and consumes the v1.18
...
before any outcome reset and permanently consumes v1.18.
...
if [ ! -d "$FORMAL_AUDIT" ]; then
  echo "formal audit failed before publishing an authenticated attempt; v1.18 is retired" >&2
  exit 2
fi
"$QA_PY" -I -B -c \
...
establishes that the sole v1.18 formal-audit claim was consumed, even if no
...
bounded WM-001 claim. Every other terminal state retires v1.18 without
