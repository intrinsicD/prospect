# WM-001 protocol 1.19.0 confirmation plan

Status: prospective. This document must be sealed with the v1.19 protocol,
schemas, implementation, dependency lock, tests, and independent harness review
before any v1.19 development or formal outcome is produced.

Protocols 1.10.0 through 1.18.0 are immutable and terminally retired. Version
1.18 passed every prospective, development, closure, preformal, binding,
formal-input-preflight, accepted-rehearsal, and final stop/go gate. Its sole
formal producer completed under authenticated custody. The sole independent
formal audit then reached its sealed 600-second deadline before producing an
ordinary report. Its authenticated terminal records a timeout, no return code,
zero stdout bytes, zero stderr bytes, and no reviewed gate. The independent
semantic review rejected the attempt, and adjudication performed zero replay
and published a rejected outer-finalized package.

The immediate failure was the deadline. The deeper harness defect was that the
result-free and small-fixture gates never related the full auditor's declared
byte ceilings and repeated result work to a production-scale liveness budget.
Every v1.18 producer, audit-failure, review, and adjudication byte remains
immutable and cannot be removed, resumed, upgraded, repaired, or reused. No
v1.18 K3–K6 value or other scientific performance value was manually opened or
used to select the repair. The exact disposition is preserved in the
[v1.18 formal-audit timeout record](wm001-v1180-formal-audit-timeout.md).

The v1.19 protocol directly supersedes the exact sealed v1.18 protocol bytes
with SHA-256
...
WM-001 v1.19.0 permits one fresh-seed confirmation of the unchanged
...
It changes only the audit-liveness contract, its independently recomputed
evidence, and the test composition needed to repair that defect:

1. audit-runtime-manifest v2 has a closed `execution_role`: result-free
   `conformance` retains exactly 600 seconds, while every full development,
   formal, and adjudication-replay `outcome_audit` receives exactly 10,800
   seconds;
2. captured-audit-execution v2 adds the successful child process's
   strict-positive `subprocess_elapsed_ns`; the runner measures this around the
   isolated subprocess and no caller may supply or rewrite it;
3. audit-reproduction v3 binds both byte-identical development executions,
   their elapsed times, the exact producer-manifest identity, and one nested
   audit-capacity v1 record;
4. capacity calibration uses the slower of the first full development audit
   and its byte-identical replay. It separately integer-ceils a two-times
   aggregate projection to 8 GiB and a two-times result reread/materialization
   projection to 2 GiB, then requires their sum to fit 10,800 seconds;
5. the full auditor's fixed-workload result and aggregate producer ceilings are
   reduced from 4 GiB and 16 GiB to 2 GiB and 8 GiB. The generic 4 GiB
   per-file custody ceiling, archive bounds, result-qualification joins, and
   existing adversarial checks remain;
6. the runner and embedded bootstrap enforce the closed runtime role and its
   timeout, and the runner alone measures child elapsed time. The operator and
   binding path preserve and join those receipts. The launcher, central
   verifier, independent auditor, and adjudicator reopen the exact
   runtime/invocation, producer manifest, and both elapsed receipts and
   independently recompute capacity;
7. formal-binding v10, raw-result v9, protocol v9, prebinding v2, preformal v2,
   closure v2, runtime-seal v1, formal-input-preflight v1, rehearsal v1, and
   formal-launch v3 remain unchanged; audit-runtime-manifest advances to v2,
   captured-audit-execution to v2, audit-reproduction to v3, and
   adjudication-audit-execution to v3;
8. the central verifier and independent auditor retain exact historical tuple
   equality and add v1.18 to the prior master/stream universe, producing 180
   prior masters and 24,480 unique prior streams with zero collision in every
   declared class; and
...
The v1.19 repair may change only runtime-role binding, timeout propagation,
elapsed-time custody, capacity arithmetic, fixed-workload byte ceilings,
their regressions, versioned paths, fresh seeds, and the evidence plumbing
required for those roles. It does not change formal-binding v10 or raw-result
v9. It may not change the world model, learning algorithm,
...
- `experiment.revision.supersedes` is exactly `1.18.0` and its pinned
...
- every v1.19 protocol, verifier, auditor, prospective review, and result schema
  agrees on the new version, paths, seeds, and exact support-file set.

The v1.10 through v1.18 failures are engineering lineage only. They supply no
evidence for or against the scientific claim.

## Assurance boundary

Every v1.19 seal, binding, operator record, audit receipt, and adjudication
...
```text
SHA256("WM-001|1.19.0|<lane>-master|<i>")
```

Development indices 0–1 are:

```text
2548769521, 799442746
```

Formal indices 0–7 are:

```text
3714505505, 79878112, 795255854, 1251505627,
1184933223, 3676873506, 286726369, 2337061326
```
...
- 180 unique exposed prior masters and 24,480 unique exposed prior streams from
  v1.0.0, v1.2.0, v1.3.0, v1.4.0, v1.5.0, v1.6.0, v1.7.0, v1.8.0,
  v1.9.0, v1.10.0, v1.11.0, v1.12.0, v1.13.0, v1.14.0, v1.15.0,
  v1.16.0, v1.17.0, and v1.18.0; and
- zero overlap in all current/prior master/stream cross classes.

No seed is replaced after any v1.19 outcome-producing path is created.

## Exact runtime and environment

The QA and producer environments are fresh, separate, non-editable virtual
environments at
`/home/alex/.venvs/prospect-wm001-v119-reviewed` and
`/home/alex/.venvs/prospect-wm001-v119-reviewed-runtime`. Both install the one
reviewed wheel built in a fresh `prospect-wm001-v119-wheelhouse.*` directory.
...
trusting its `passed` member. A failed capacity gate retires v1.19 before
...
The retained result qualification must name the exact v1.19 development lane,
...
v1.19 canonical root, and preflight inputs are reopened through stable,
...
There is exactly one v1.19 development producer:

```text
bench/world_model_lifecycle/results/development/qualification-v1.19.0
```

Exclusive creation of that root consumes the v1.19 development qualification.
...
`results/development/diagnostics-v1.19.0`; they cannot occupy any lifecycle
namespace. The sole canonical development audit and closure paths are:

```text
bench/world_model_lifecycle/results/operator-v1.19/audits/development-audit-v1.19.0
bench/world_model_lifecycle/results/operator-v1.19/closures/development-closure-v1.19.0
bench/world_model_lifecycle/results/development/development-closure-v1.19.0.json
```
...
failure—retires v1.19. A terminal failure does not restore authorization merely
...
```text
bench/world_model_lifecycle/results/development/v1.19.0/preformal
```
...
the hidden claim; it still consumes v1.19 and cannot be deleted and retried.
...
```text
bench/world_model_lifecycle/results/operator-v1.19/bindings/formal-binding-v1.19.0
```
...
`results/rehearsals/v1.19/accepted-binding-<binding-sha256>.json` namespace.
...
```text
bench/world_model_lifecycle/results/formal/formal-launch-v1.19.0.json
```
