# WM-001 v1.11.0 operator runbook

Status: prospective. Seal this runbook with the exact v1.11 protocol,
implementation, schemas, lock, tests, and independent review before any v1.11
producer path is created.

Protocol 1.10.0 is terminally retired. Its sole producer, independent audit,
closure transaction, retained fresh-runtime receipt, same-inode outer
completion, and post-finalization sealed-runtime reopen completed. Its
preformal generator ran all ten commands but then re-entered a live
runtime-inventory verifier from the intentionally larger QA environment. The
ordinary exception escaped before report publication, leaving a terminal
hidden 20-log claim that authorized no binding or formal launch. Do not
inspect, summarize, compare, or use any v1.10 K3–K6 or other performance
value. Nothing in this runbook repairs, resumes, or reuses v1.10 evidence.
...
QA_PY="/home/alex/.venvs/prospect-wm001-v111-reviewed/bin/python"
RUNTIME_PY="/home/alex/.venvs/prospect-wm001-v111-reviewed-runtime/bin/python"
...
DEV="$DEV_ROOT/qualification-v1.11.0"
RUNTIME_SEAL="$DEV_ROOT/runtime-seal-v1.11.0.json"
DEV_CLOSURE="$DEV_ROOT/development-closure-v1.11.0.json"
PREFORMAL_ROOT="$DEV_ROOT/v1.11.0/preformal"
PREFORMAL="$PREFORMAL_ROOT/preformal-test-report-v1.11.0.json"

OPERATOR_ROOT="$REPO/bench/world_model_lifecycle/results/operator-v1.11"
DEV_AUDIT="$OPERATOR_ROOT/audits/development-audit-v1.11.0"
CLOSURE_ATTEMPT="$OPERATOR_ROOT/closures/development-closure-v1.11.0"
BINDING_ATTEMPT="$OPERATOR_ROOT/bindings/formal-binding-v1.11.0"
BINDING="$BINDING_ATTEMPT/formal-binding.json"
FORMAL_INPUT_PREFLIGHT="$BINDING_ATTEMPT/formal-input-preflight.json"

OUTER_ROOT="$REPO/bench/world_model_lifecycle/results/outer-completions/v1.11"
RUNTIME_LOCK="$REPO/bench/world_model_lifecycle/results/.wm001-v1.11-runtime.lock"
FORMAL_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-launch-v1.11.0.json"
FORMAL_AUDIT="$OPERATOR_ROOT/audits/formal-audit-v1.11.0"
FORMAL_AUDIT_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-audit-v1.11.0.json"
ADJUDICATION_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-adjudication-v1.11.0.json"
ADJUDICATION_ROOT="$REPO/bench/world_model_lifecycle/results/adjudication-v1.11"
ADJUDICATION_PACKAGE="$ADJUDICATION_ROOT/formal-adjudication-v1.11.0"

REVIEW="$REPO/docs/wm001-v1110-prospective-harness-review.json"
SEMANTIC_REVIEW="$REPO/artifacts/wm001-reviews/formal-v1.11.0.json"
...
The development masters are exactly `670819759, 624845448`. The formal
masters are exactly `3391764770, 20596598, 999954271, 2371040464,
2073495343, 962058337, 2170781413, 3523651983`. The verifier must verify 100
unique prior masters and 13,600 unique prior streams through v1.10 before any
evidence run.
...
WHEELHOUSE="$(mktemp -d /tmp/prospect-wm001-v111-wheelhouse.XXXXXX)"
...
generated WM-001 header must name v1.11.0. The Prospect row must equal the
...
lines = [
    "# WM-001 protocol 1.11.0 isolated live execution closure.",
    "# Versions preserve the v1.4 scientific runtime; each v1.11 digest covers",
...
`wm001-v1110-prospective-harness-review.json`, then commit the whole candidate.
...
- 10 current masters, 1,360 current streams, 100 prior masters, and 13,600
...
v1.11 producer root. Verify the exact real-subprocess branch tests explicitly
...
the v1.11 development qualification even if any later phase fails.
...
or missing ordinary audit retires v1.11; never rerun it.
...
binding, or stop/go failure retires v1.11. Absence of a downstream path never
...
`$DEV_ROOT/v1.11.0/.preformal.staging`, then fsyncs its parent before its first
...
```bash
require_absent "$PREFORMAL_ROOT"
require_absent "$DEV_ROOT/v1.11.0/.preformal.staging"
...
either path or retry v1.11. A successful generation is not sufficient by
...
version `1.11.0`, `passed: true`, and digests for the binding, preformal report,
development closure, accepted-closure evidence, runtime conformance, and
auditor source. Any missing, false, stale, or mismatched receipt retires v1.11.
...
FORMAL="$FORMAL_BINDING_ROOT/confirmation-v1.11.0"
...
the v1.11 attempt. If either canonical path exists, the attempt is likewise
...
if [ ! -d "$FORMAL_AUDIT" ]; then
  echo "formal audit failed before publishing an authenticated attempt; v1.11 is retired" >&2
  exit 2
fi
"$QA_PY" -I -B -c \
...
establishes that the sole v1.11 formal-audit claim was consumed, even if no
...
bounded WM-001 claim. Every other terminal state retires v1.11 without
