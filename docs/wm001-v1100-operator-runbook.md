# WM-001 v1.10.0 operator runbook

Status: prospective. Seal this runbook with the exact v1.10 protocol,
implementation, schemas, lock, tests, and independent review before any v1.10
producer path is created.

Protocol 1.9.0 is terminally retired. Its sole producer, independent audit,
closure transaction, retained fresh-runtime receipt, same-inode outer
completion, and post-finalization sealed-runtime reopen completed. Its fixed
preformal gate then failed because one runner test redirected the producer root
but still observed the real canonical closure path. The failed report
authorized no binding or formal launch. Do not inspect, summarize, compare, or
use any v1.9 K3–K6 value. Nothing in this runbook repairs or reuses v1.9
evidence.
...
QA_PY="/home/alex/.venvs/prospect-wm001-v110-reviewed/bin/python"
RUNTIME_PY="/home/alex/.venvs/prospect-wm001-v110-reviewed-runtime/bin/python"
...
DEV="$DEV_ROOT/qualification-v1.10.0"
RUNTIME_SEAL="$DEV_ROOT/runtime-seal-v1.10.0.json"
DEV_CLOSURE="$DEV_ROOT/development-closure-v1.10.0.json"
PREFORMAL_ROOT="$DEV_ROOT/v1.10.0/preformal"
PREFORMAL="$PREFORMAL_ROOT/preformal-test-report-v1.10.0.json"

OPERATOR_ROOT="$REPO/bench/world_model_lifecycle/results/operator-v1.10"
DEV_AUDIT="$OPERATOR_ROOT/audits/development-audit-v1.10.0"
CLOSURE_ATTEMPT="$OPERATOR_ROOT/closures/development-closure-v1.10.0"
BINDING_ATTEMPT="$OPERATOR_ROOT/bindings/formal-binding-v1.10.0"
BINDING="$BINDING_ATTEMPT/formal-binding.json"
FORMAL_INPUT_PREFLIGHT="$BINDING_ATTEMPT/formal-input-preflight.json"

OUTER_ROOT="$REPO/bench/world_model_lifecycle/results/outer-completions/v1.10"
RUNTIME_LOCK="$REPO/bench/world_model_lifecycle/results/.wm001-v1.10-runtime.lock"
FORMAL_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-launch-v1.10.0.json"
FORMAL_AUDIT="$OPERATOR_ROOT/audits/formal-audit-v1.10.0"
FORMAL_AUDIT_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-audit-v1.10.0.json"
ADJUDICATION_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-adjudication-v1.10.0.json"
ADJUDICATION_ROOT="$REPO/bench/world_model_lifecycle/results/adjudication-v1.10"
ADJUDICATION_PACKAGE="$ADJUDICATION_ROOT/formal-adjudication-v1.10.0"

REVIEW="$REPO/docs/wm001-v1100-prospective-harness-review.json"
SEMANTIC_REVIEW="$REPO/artifacts/wm001-reviews/formal-v1.10.0.json"
...
The development masters are exactly `1647437737, 1156509260`. The formal
masters are exactly `3363134750, 2153178322, 2277484641, 572614265,
3119775486, 3121614244, 3646941950, 827253974`. The verifier must verify 90
unique prior masters and 12,240 unique prior streams through v1.9 before any
evidence run.
...
WHEELHOUSE="$(mktemp -d /tmp/prospect-wm001-v110-wheelhouse.XXXXXX)"
...
generated WM-001 header must name v1.10.0. The Prospect row must equal the
...
lines = [
    "# WM-001 protocol 1.10.0 isolated live execution closure.",
    "# Versions preserve the v1.4 scientific runtime; each v1.10 digest covers",
...
`wm001-v1100-prospective-harness-review.json`, then commit the whole candidate.
...
- 10 current masters, 1,360 current streams, 90 prior masters, and 12,240
...
v1.10 producer root. Verify the exact real-subprocess branch tests explicitly
...
the v1.10 development qualification even if any later phase fails.
...
or missing ordinary audit retires v1.10; never rerun it.
...
binding, or stop/go failure retires v1.10. Absence of a downstream path never
...
`$DEV_ROOT/v1.10.0/.preformal.staging`, then fsyncs its parent before its first
...
```bash
require_absent "$PREFORMAL_ROOT"
require_absent "$DEV_ROOT/v1.10.0/.preformal.staging"
...
either path or retry v1.10. A successful generation is not sufficient by
...
command 10 and requires each semantic-object digest to equal the corresponding
receipt digest. The receipt must have schema
`prospect.wm001.formal-input-preflight.v1`, protocol
version `1.10.0`, `passed: true`, and digests for the binding, preformal report,
development closure, accepted-closure evidence, runtime conformance, and
auditor source. Any missing, false, stale, or mismatched receipt retires v1.10.
...
FORMAL="$FORMAL_BINDING_ROOT/confirmation-v1.10.0"
...
the v1.10 attempt. If either canonical path exists, the attempt is likewise
...
if [ ! -d "$FORMAL_AUDIT" ]; then
  echo "formal audit failed before publishing an authenticated attempt; v1.10 is retired" >&2
  exit 2
fi
"$QA_PY" -I -B -c \
...
establishes that the sole v1.10 formal-audit claim was consumed, even if no
...
bounded WM-001 claim. Every other terminal state retires v1.10 without
