# WM-001 v1.14.0 operator runbook

Status: prospective. Seal this runbook with the exact v1.14 protocol,
implementation, schemas, lock, tests, and independent review before any v1.14
producer path is created.

Protocols 1.10.0 through 1.13.0 are terminally retired. Version 1.10 failed at
preformal on an unequal QA/runtime composition. Version 1.11 failed its
result-free exact-zero-stderr rehearsal because deprecated TF32 access emitted
a warning. Version 1.12 repaired those classes but its binding schema rejected
the ten correctly empty stderr identities. Version 1.13 repaired that schema,
passed command 10, completed and outer-finalized development, accepted audit,
closure, and all ten preformal commands, and assembled a root-schema-valid
binding package.

The v1.13 strict binding consumer then invoked the canonical live-bundle
report verifier on the preserved report inside the mixed binding staging
directory. The canonical role requires the original development/preformal path
and an exclusive report-plus-20-logs directory, so it rejected the valid
archive role before binding acceptance. The failed binding attempt was
outer-finalized. No v1.13 formal launch or formal outcome exists. Do not
inspect or use any retained K or performance value. Nothing in this runbook
repairs, resumes, removes, or reuses v1.10 through v1.13 evidence,
environments, wheels, seals, locks, bindings, or outer completions. The
immediate disposition is documented in the
[v1.13 binding-verifier failure](wm001-v1130-binding-verifier-failure.md).

The active protocol must directly supersede v1.13.0 and pin its exact sealed
protocol SHA-256 as
...
QA_PY="/home/alex/.venvs/prospect-wm001-v114-reviewed/bin/python"
RUNTIME_PY="/home/alex/.venvs/prospect-wm001-v114-reviewed-runtime/bin/python"
...
DEV="$DEV_ROOT/qualification-v1.14.0"
DEV_DIAGNOSTICS="$DEV_ROOT/diagnostics-v1.14.0"
RUNTIME_SEAL="$DEV_ROOT/runtime-seal-v1.14.0.json"
DEV_CLOSURE="$DEV_ROOT/development-closure-v1.14.0.json"
PREFORMAL_ROOT="$DEV_ROOT/v1.14.0/preformal"
PREFORMAL_CLAIM="$DEV_ROOT/v1.14.0/.preformal.staging"
PREFORMAL="$PREFORMAL_ROOT/preformal-test-report-v1.14.0.json"

OPERATOR_ROOT="$REPO/bench/world_model_lifecycle/results/operator-v1.14"
DEV_AUDIT="$OPERATOR_ROOT/audits/development-audit-v1.14.0"
CLOSURE_ATTEMPT="$OPERATOR_ROOT/closures/development-closure-v1.14.0"
BINDING_ATTEMPT="$OPERATOR_ROOT/bindings/formal-binding-v1.14.0"
BINDING="$BINDING_ATTEMPT/formal-binding.json"
FORMAL_INPUT_PREFLIGHT="$BINDING_ATTEMPT/formal-input-preflight.json"

OUTER_ROOT="$REPO/bench/world_model_lifecycle/results/outer-completions/v1.14"
RUNTIME_LOCK="$REPO/bench/world_model_lifecycle/results/.wm001-v1.14-runtime.lock"
FORMAL_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-launch-v1.14.0.json"
FORMAL_AUDIT="$OPERATOR_ROOT/audits/formal-audit-v1.14.0"
FORMAL_AUDIT_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-audit-v1.14.0.json"
ADJUDICATION_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-adjudication-v1.14.0.json"
ADJUDICATION_ROOT="$REPO/bench/world_model_lifecycle/results/adjudication-v1.14"
ADJUDICATION_PACKAGE="$ADJUDICATION_ROOT/formal-adjudication-v1.14.0"

REVIEW="$REPO/docs/wm001-v1140-prospective-harness-review.json"
SEMANTIC_REVIEW="$REPO/artifacts/wm001-reviews/formal-v1.14.0.json"
...
require_v114_lifecycle_absent() {
  ...
  if find "$REPO/bench/world_model_lifecycle/results/formal" \
      -mindepth 1 -name '*v1.14.0*' -print -quit 2>/dev/null |
      grep -q .
  then
    echo "unexpected v1.14 formal lifecycle path exists" >&2
    return 2
  fi
}
```

The development masters are exactly `630481329, 2204125221`. The formal
masters are exactly `900802928, 2035185068, 3817247901, 14769188,
2670334085, 2866408483, 671156171, 333753598`. The verifier must verify 130
unique prior masters and 17,680 unique prior streams through v1.13 before any
evidence run.
...
WHEELHOUSE="$(mktemp -d /tmp/prospect-wm001-v114-wheelhouse.XXXXXX)"
...
generated WM-001 header must name v1.14.0. The Prospect row must equal the
...
lines = [
    "# WM-001 protocol 1.14.0 isolated live execution closure.",
    "# Versions preserve the v1.4 scientific runtime; each v1.14 digest covers",
...
`wm001-v1140-prospective-harness-review.json`, then commit the whole candidate.
...
"$QA_PY" -I -B -m pytest -q \
  ...
  -k 'preserved_preformal or binding_archive'
...
The targeted archive-role tests are a mandatory producer-to-consumer gate, not
name-only unit coverage. They must generate one valid canonical preformal
...
- the canonical verifier accepts only the original v1.14 development bundle
...
- the preserved verifier rejects a missing or changed referenced stream and
  any additional report or `preformal-v1.14.0-command-*` member; and
...
- direct supersession of v1.13.0 protocol SHA-256
  `e7988e3605079b7b7830949d6fd107f26066059ac3cc3974c5bfe15af876dc0c`;
- scientific-block SHA-256
  `fa44fd93a672db3905d45a0e99c568985e7e2e5d02d32043c830db413005a5c3`;
- 10 current masters, 1,360 current streams, 130 prior masters, and 17,680
...
- unchanged raw-result v9 and formal-binding v10 representation identities,
  with only the v1.14 protocol and formal-seed constants updated; and
...
```bash
COMMAND10_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v114-command10.XXXXXX)"
...
if [ "$COMMAND10_RC" -ne 0 ] || [ -s "$COMMAND10_STDERR" ]; then
  echo "result-free command 10 failed or wrote stderr; v1.14 is retired" >&2
...
stderr retires v1.14; do not repair its source, wheel, environment, seal, or
capture in place.
...
v1.14 producer root. Verify the exact real-subprocess branch tests explicitly
...
nonzero return, or nonempty stderr consumes and retires v1.14; do not repeat
...
the v1.14 development qualification even if any later phase fails.
...
or missing ordinary audit retires v1.14; never rerun it.
...
binding, or stop/go failure retires v1.14. Absence of a downstream path never
...
`$DEV_ROOT/v1.14.0/.preformal.staging`, then fsyncs its parent before its first
...
require_absent "$DEV_ROOT/v1.14.0/.preformal.staging"
...
either path or retry v1.14. A successful generation is not sufficient by
...
version `1.14.0`, `passed: true`, and digests for the binding, preformal report,
development closure, accepted-closure evidence, runtime conformance, and
auditor source. Any missing, false, stale, or mismatched receipt retires v1.14.
...
FORMAL="$FORMAL_BINDING_ROOT/confirmation-v1.14.0"
...
sibling left by an earlier launcher, contaminates and consumes the v1.14
...
if [ ! -d "$FORMAL_AUDIT" ]; then
  echo "formal audit failed before publishing an authenticated attempt; v1.14 is retired" >&2
  exit 2
fi
"$QA_PY" -I -B -c \
...
establishes that the sole v1.14 formal-audit claim was consumed, even if no
...
bounded WM-001 claim. Every other terminal state retires v1.14 without
