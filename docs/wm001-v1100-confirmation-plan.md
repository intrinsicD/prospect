# WM-001 protocol 1.10.0 confirmation plan

Status: prospective. This document must be sealed with the v1.10 protocol,
schemas, implementation, dependency lock, tests, and independent harness review
before any v1.10 development or formal outcome is produced.

Protocol 1.9.0 is immutable and terminally retired. Its sole development
producer, independent audit, closure transaction, retained fresh-runtime
receipt, same-inode outer completion, and post-finalization sealed-runtime
reopen completed. Its fixed preformal gate then failed because one runner test
redirected the producer root and qualification path but still observed the
real canonical development-closure path. The same test had passed before that
path existed. No v1.9 binding, formal launch, formal audit, semantic review, or
adjudication exists. No v1.9 K3–K6 value was opened and none may be inspected,
summarized, compared, or used to select a v1.10 setting.

## Purpose and scientific freeze

WM-001 v1.10.0 permits one fresh-seed confirmation of the unchanged
collect → learn → improve → retain → restart experiment. It retains the
v1.9 canonical matrix contract and its digest
...
1. runner lifecycle paths are module-level, root-derived, and hermetically
   redirected by tests;
2. seed-override diagnostics are confined to an automatically named,
   version-owned namespace and cannot occupy lifecycle paths;
3. every operator stage has a deterministic, race-safe hidden one-shot claim,
   and a canonical closure marker can never be used to resume closure;
4. preformal uses its own deterministic hidden claim, keeps the canonical
   bundle absent while all ten commands run, and atomically publishes the
   complete 21-file directory;
5. a failed report produces a truthful nonzero generation envelope identifying
   command, identity, and semantic runtime-output failures separately;
6. the obsolete public `development-evidence` runtime mode is removed;
7. the producer and independent auditor directly agree on command 9,
   `runtime-accepted-closure-evidence`, its seven inputs, semantic digest
   checks, and same-inode two-link closure-attempt custody; and
8. the exact independent formal-input consumer must pass before binding
   acceptance, its canonical receipt is retained for later verification and
   formal audit, and the outer launcher requires and cross-checks that receipt
   before importing the formal producer.
...
- every v1.10 protocol, verifier, auditor, prospective review, and result schema
  agrees on the new version, paths, seeds, and exact support-file set.

The v1.9 preformal failure is engineering lineage only. It supplies no evidence
for or against the scientific claim.

## Assurance boundary

Every v1.10 seal, binding, operator record, audit receipt, and adjudication
...
```text
SHA256("WM-001|1.10.0|<lane>-master|<i>")
```

Development indices 0–1 are:

```text
1647437737, 1156509260
```
...
- 90 unique exposed prior masters and 12,240 unique exposed prior streams from
  v1.0.0, v1.2.0, v1.3.0, v1.4.0, v1.5.0, v1.6.0, v1.7.0, v1.8.0, and
  v1.9.0; and
- zero overlap in all current/prior master/stream cross classes.

No seed is replaced after any v1.10 outcome-producing path is created.

## Exact runtime and environment

The QA and producer environments are fresh, separate, non-editable virtual
environments at
`/home/alex/.venvs/prospect-wm001-v110-reviewed` and
`/home/alex/.venvs/prospect-wm001-v110-reviewed-runtime`. Both install the one
reviewed wheel built in a fresh `prospect-wm001-v110-wheelhouse.*` directory.
...
There is exactly one v1.10 development producer:

```text
bench/world_model_lifecycle/results/development/qualification-v1.10.0
```

Exclusive creation of that root consumes the v1.10 development qualification.
...
`results/development/diagnostics-v1.10.0`; they cannot occupy any lifecycle
namespace. The sole canonical development audit and closure paths are:

```text
bench/world_model_lifecycle/results/operator-v1.10/audits/development-audit-v1.10.0
bench/world_model_lifecycle/results/operator-v1.10/closures/development-closure-v1.10.0
bench/world_model_lifecycle/results/development/development-closure-v1.10.0.json
```
...
failure—retires v1.10. A terminal failure does not restore authorization merely
...
```text
bench/world_model_lifecycle/results/development/v1.10.0/preformal
```
...
the hidden claim; it still consumes v1.10 and cannot be deleted and retried.
...
`development-evidence` mode. Command 10 remains the sealed result-free
bootstrap/inventory conformance rehearsal.
...
```text
bench/world_model_lifecycle/results/operator-v1.10/bindings/formal-binding-v1.10.0
```
...
```text
bench/world_model_lifecycle/results/formal/formal-launch-v1.10.0.json
```
