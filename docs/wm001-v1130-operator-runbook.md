# WM-001 v1.13.0 operator runbook

Status: prospective. Seal this runbook with the exact v1.13 protocol,
implementation, schemas, lock, tests, and independent review before any v1.13
producer path is created.

Protocols 1.10.0, 1.11.0, and 1.12.0 are terminally retired. Version 1.10
failed at preformal on an unequal QA/runtime composition. Version 1.11 failed
its result-free exact-zero-stderr rehearsal because deprecated TF32 access
emitted a warning. Version 1.12 repaired both classes, passed command 10,
completed and outer-finalized development, an accepted audit, closure, and all
ten preformal commands, then failed its sole binding transaction.

The v1.12 formal-binding schema reused `fileDigest` with `bytes.minimum: 1`
for the 20 preformal stream rows even though all ten stderr rows were correctly
and necessarily empty. Schema validation rejected the first `bytes: 0`, and
the failed binding attempt was outer-finalized. No v1.12 formal launch or
formal outcome exists. Do not inspect or use any retained K or performance
value. Nothing in this runbook repairs, resumes, removes, or reuses v1.10,
v1.11, or v1.12 evidence, environments, wheels, seals, locks, bindings, or
outer completions. The immediate disposition is documented in the
...
QA_PY="/home/alex/.venvs/prospect-wm001-v113-reviewed/bin/python"
RUNTIME_PY="/home/alex/.venvs/prospect-wm001-v113-reviewed-runtime/bin/python"
...
DEV="$DEV_ROOT/qualification-v1.13.0"
RUNTIME_SEAL="$DEV_ROOT/runtime-seal-v1.13.0.json"
DEV_CLOSURE="$DEV_ROOT/development-closure-v1.13.0.json"
PREFORMAL_ROOT="$DEV_ROOT/v1.13.0/preformal"
PREFORMAL="$PREFORMAL_ROOT/preformal-test-report-v1.13.0.json"

OPERATOR_ROOT="$REPO/bench/world_model_lifecycle/results/operator-v1.13"
DEV_AUDIT="$OPERATOR_ROOT/audits/development-audit-v1.13.0"
CLOSURE_ATTEMPT="$OPERATOR_ROOT/closures/development-closure-v1.13.0"
BINDING_ATTEMPT="$OPERATOR_ROOT/bindings/formal-binding-v1.13.0"
BINDING="$BINDING_ATTEMPT/formal-binding.json"
FORMAL_INPUT_PREFLIGHT="$BINDING_ATTEMPT/formal-input-preflight.json"

OUTER_ROOT="$REPO/bench/world_model_lifecycle/results/outer-completions/v1.13"
RUNTIME_LOCK="$REPO/bench/world_model_lifecycle/results/.wm001-v1.13-runtime.lock"
FORMAL_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-launch-v1.13.0.json"
FORMAL_AUDIT="$OPERATOR_ROOT/audits/formal-audit-v1.13.0"
FORMAL_AUDIT_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-audit-v1.13.0.json"
ADJUDICATION_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-adjudication-v1.13.0.json"
ADJUDICATION_ROOT="$REPO/bench/world_model_lifecycle/results/adjudication-v1.13"
ADJUDICATION_PACKAGE="$ADJUDICATION_ROOT/formal-adjudication-v1.13.0"

REVIEW="$REPO/docs/wm001-v1130-prospective-harness-review.json"
SEMANTIC_REVIEW="$REPO/artifacts/wm001-reviews/formal-v1.13.0.json"
...
The development masters are exactly `560818116, 1392377688`. The formal
masters are exactly `140647545, 2239253745, 3333612762, 4269572592,
2151457732, 4034984701, 2426483518, 2833322658`. The verifier must verify 120
unique prior masters and 16,320 unique prior streams through v1.12 before any
evidence run.
...
WHEELHOUSE="$(mktemp -d /tmp/prospect-wm001-v113-wheelhouse.XXXXXX)"
...
generated WM-001 header must name v1.13.0. The Prospect row must equal the
...
lines = [
    "# WM-001 protocol 1.13.0 isolated live execution closure.",
    "# Versions preserve the v1.4 scientific runtime; each v1.13 digest covers",
...
`wm001-v1130-prospective-harness-review.json`, then commit the whole candidate.
...
- 10 current masters, 1,360 current streams, 120 prior masters, and 16,320
...
```bash
COMMAND10_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v113-command10.XXXXXX)"
...
if [ "$COMMAND10_RC" -ne 0 ] || [ -s "$COMMAND10_STDERR" ]; then
  echo "result-free command 10 failed or wrote stderr; v1.13 is retired" >&2
...
stderr retires v1.13; do not repair its source, wheel, environment, seal, or
capture in place.
...
v1.13 producer root. Verify the exact real-subprocess branch tests explicitly
...
nonzero return, or nonempty stderr consumes and retires v1.13; do not repeat
...
the v1.13 development qualification even if any later phase fails.
...
or missing ordinary audit retires v1.13; never rerun it.
...
binding, or stop/go failure retires v1.13. Absence of a downstream path never
...
`$DEV_ROOT/v1.13.0/.preformal.staging`, then fsyncs its parent before its first
...
```bash
require_absent "$PREFORMAL_ROOT"
require_absent "$DEV_ROOT/v1.13.0/.preformal.staging"
...
either path or retry v1.13. A successful generation is not sufficient by
...
Before the operator creates the deterministic binding claim, it must validate
the actual 20-row `preformal_log_rows()` projection against the root
formal-binding v10 schema and its referenced definitions. Ten stdout rows are
nonempty and all ten stderr rows are exact empty-stream identities. This
preclaim check must also prove that implementation files still use the
nonempty `fileDigest` definition. After the complete binding object is
assembled, the producer must validate that full object against the root schema
before publishing any binding sidecar or `formal-binding.json`; the later
independent verifier remains mandatory.
...
version `1.13.0`, `passed: true`, and digests for the binding, preformal report,
development closure, accepted-closure evidence, runtime conformance, and
auditor source. Any missing, false, stale, or mismatched receipt retires v1.13.
...
FORMAL="$FORMAL_BINDING_ROOT/confirmation-v1.13.0"
...
the v1.13 attempt. If either canonical path exists, the attempt is likewise
...
if [ ! -d "$FORMAL_AUDIT" ]; then
  echo "formal audit failed before publishing an authenticated attempt; v1.13 is retired" >&2
  exit 2
fi
"$QA_PY" -I -B -c \
...
establishes that the sole v1.13 formal-audit claim was consumed, even if no
...
bounded WM-001 claim. Every other terminal state retires v1.13 without
