# MM-011 causal deformation/appearance prediction protocol
...
exclusive `freeze-record.json` is durable. No MM-011 real prediction or score may be
computed before that event.  
**Parent:** canonical LCV-001 `PASS`, containing the exact sealed MM-007 consumer
surface and host-bound runtime receipt  
**Scientific provenance:** MM-008 v2.2's development-validated finite-grid
mathematical sources and MM-009's pre-real-validated scientific grammar. MM-009 is
terminal-invalid and supplies no real outcome or execution authority. MM-011 is the
first possible valid scientific execution, not a retry or repair.

## 1. Question and claim boundary

MM-011 asks whether an operator estimated only from the observed half-second change
...
## 2. Immutable LCV parent and source closure

Before the formal marker, preparation authenticates and copies the complete canonical
LCV-001 package as opaque filesystem objects under `inputs/LCV-001`. The source must
have exactly 42 regular single-link files at `0444`, 17 real directories at `0555`,
7,821,446 total file bytes, and the exact self-excluding artifact manifest. The copied
package must independently pass LCV's structural verifier. The following bindings are
mandatory:

```text
LCV result                 d7e403008239d57b7f662f28cdb686a851f3cdd3c7164be4ff423bdeeabe1e94
LCV artifact manifest      1ea64a430143315f9dd64c4f211a01d418d3a568d874cdef7d62943b27d0de5e
LCV parent closure         bf47ef0e18c13885c943d29939723791a1084c28b66572b876723f4ddf29d005
LCV controls               2eb640ac9991c6543dd9846ac0781eec6577af061dabf0dd74b69f578e8048d5
LCV runtime receipt        29df8291ddaf0f6e93c562686a6ad678716fd9d341d7508c55f725a818ee25b1
LCV cleanup receipt        60f8c1f97f560d6c83ed8fff22183dd3528d60233b55fa20e37ff6acc3f0bce3
```

Preparation must not decode the embedded frame NPZ, parse its scientific evidence,
invoke a historical MM-001--MM-007 verifier, or construct a real row. It may parse
only LCV's own manifest/result/runtime/cleanup grammar after the exact tree snapshot
has been authenticated. The eight successor-facing MM-007 files below are consumed
only from the copied path `inputs/LCV-001/prepared/inputs/MM-007/`; their bytes retain
the registered scientific identities, while the enclosing LCV copy has sealed every
file as `0444`:

```text
artifact-manifest.json     db0b6654ab098dc9a3ec93e4a6de8820bbe5860d44974645e9a5ee7dad1537fb  0444
input-manifest.json        1f83c805e6c5d75f4f1d5a2102d471c15bbc6bb787960cb5ae630bd2260faa1f  0444
formal-start.json          ea5c7bda870d71ead3172c1fc6e504d6a6b02d2ba785e9fd2fc75a91c667eee3  0444
MM-007-evidence.json       13dfa89e541e6122263ea9814d42fb328da303dcc74556cdaaa5d5860d99abaf  0444
MM-007-results.json        3c92729e1e5c18c14461e36602bdb86acd31750d9f5a85f535cd33a43fb9c47b  0444
MM-007-report.md           b18760128941ab2eff893b8c0afc469b92f71077d489e060d56519407990b8a2  0444
MM-007-protocol.md         24bbac1855cc2b51d2a65012b9c63037637c53555b86bbad7c66a6249108a73c  0444
MM-007-frames-64x64.npz    fbc79d81a06720175139f7106745bd58f8788f43cc5a2fcd10658d186909797f  0444
```
...
authenticate historical fits only. MM-011 issues separate apply and prediction
identities and never calls an inherited reconstruction a forecast. The reviewed
MM-011 source closure must reject imports of every historical experiment/verifier
module. LCV's own verifier and custody modules are the only permitted parent-validation
dependencies.

Only after the exclusive formal marker, formal MM-011 synthetic gate, copied-LCV
structural verification, and in-child runtime revalidation are durable may MM-011
parse LCV's copied successor-facing closure, evidence, or frame archive. It then uses
MM-011-owned code to replay the exact rows and normalizers. Any LCV tree, receipt,
runtime, copied-byte, row, or normalizer mismatch terminates the one-shot output as
`invalid_MM011`; it cannot be repaired or retried in place.
...
Each MM-011 apply record binds the historical fit identity, selected state and
...
`invalid_MM011_real_negative_control`. A count of 3--5/8 globally produces
`MM011_inconclusive` with a family/null-specific reason. There is no separate
`inconclusive_MM011_real_negative_control` label. Null counts of 0--2 do not preempt.
...
The formal MM-011 synthetic panel uses only these exposed seeds:
...
v2.2 and focused MM-011 unit/mutation tests before freeze; those tests are not formal
MM-011 synthetic evidence. No acceleration or periodic-ambiguity scenario is part of
the formal MM-011 control panel.

Before the implementation may freeze, a separate generated sensitivity audit uses
fresh seeds `991100..991102` and reads no real MM-007/LCV frame or future. For each
seed, `PCG64` draws six `3x96x96` standard-normal rows and six per-channel offsets;
the raw halo is `epsilon + 0.5*offset`. The source-only v2.2 normalizer is fit to the
six central `[16:80,16:80]` crops and applied unchanged to the full halos. The
existing broadband-validity gate must pass, and row zero is used without selection.
Continuous float64 bilinear sampling has no padding, reflection, or clipping: the
halo constructs a full `64x64` current frame, and the same operator is applied once
more to current at the central `48x48` sites to construct future.

The six exact positive cells are:

```text
991100 T1 (1,-1,0,0,0,0)       991100 A1 (0,0,1,0,0,-1)
991101 T2 (2,-2,0,0,0,0)       991101 A2 (0,0,0,1,-1,0)
991102 T3 (3,-3,0,0,0,0)       991102 A3 (0,0,1,-1,1,1)
```

Every truth must be finite, boundary-safe, and absent from the canonical affine
grid. For the affine result, let `I` be previous-to-current identity SSE, `H` its
historical reconstruction SSE, `P` be current-to-future persistence SSE, `F` its
forecast SSE, `RP` be current-to-previous persistence SSE, and `R` be forecast-to-
previous SSE. Each cell independently requires `I>0.6912`, `P>0.6912`,
`1.25*H<=I`, `1.25*F<=P`, and `1.25*R>RP`, plus deep source-only replay and
bit-exact one-step forecast replay. Thus the repeated-direction forecast must improve
forward but must not cross the same margin when the previous frame is substituted as
the future. All six cells must pass; no aggregate or majority can rescue one.

The broadband identity control uses seed `991100` with previous, current, and future
identical and requires zero activity, the identity affine state, and history/forecast
SSE at most `1e-24`. The low-texture control repeats channel constants
`(-0.25,0,0.25)` and requires zero activity, ineligibility, identity affine state,
and every arm's forecast SSE at most `1e-24`. The audit is diagnostic and cannot
change formal seeds, estimator grid, thresholds, or result ladder. Any generator,
positive-cell, replay, identity, or low-texture failure abandons MM-011 before
canonical preparation and promotes a continuous-registration successor. The rule
may not be weakened, tuned away, or relabelled as formal evidence after output is
observed.
...
or source-side identity change is `invalid_MM011_future_isolation` and globally
preempts interpretation.

## 8. Custody, sandbox, and prediction freeze

Preparation may authenticate/copy the complete LCV package and run synthetic-only
tests. It may not decode the embedded real frame NPZ, parse embedded MM-007 scientific
evidence, construct a real row, or invoke a real predictor before the exclusive formal
marker and formal synthetic gate. After those records are durable, the copied LCV
package must pass its own structural verifier and the actual child must replay LCV's
runtime commitments. MM-011-owned code then validates the copied LCV parent closure,
reconstructs rows and normalizers, and writes detached row inputs. No historical
multimodal experiment/verifier may be imported. Each primary row
...
One fresh predictor process is launched per row through an MM-011-owned Linux
...
boundary; they are not presented as MM-011 scientific source. Scientific authority is
limited to the copied shadow package containing the exact four v2.2 mathematical
modules and MM-011 source-only predictor code, copied dependencies, config, and that
...
unique `mm011-custody-*` unit, `Type=exec`, `RuntimeMaxSec=14400s`,
`KillMode=control-group`, `KillSignal=SIGKILL`, `SendSIGKILL=yes`, and `Restart=no`.
The supervisor descriptor-authenticates the exact terminal CPython executable and the
literal lexical `.venv/bin/python` symlink chain separately. A stdlib-only guard starts
from the terminal executable, proves its expected cgroup and frozen environment, then
calls `execve` on that same terminal binary while preserving the literal lexical path
as `argv[0]`. Resolving or replacing the reviewed lexical command is invalid. Before
scientific imports, the child must report the lexical `sys.executable`, terminal
`/proc/self/exe`, Python/NumPy/OpenBLAS identities, one-thread policy, complete bound
source maps, and the exact LCV SVD canary.

The 14,400-second ceiling applies to the lifecycle root and therefore does not lose
time to cleanup. Cleanup is a separate at-most-30-second infrastructure finalization
tail after normal exit, timeout, or interruption; it cannot alter the scientific
classification and the classification is not actionable unless that tail proves the
unit inactive and absent.

This outer boundary contains descendants even when an inner launcher creates a new
session. A fresh known-answer service must prove that a nested `setsid` descendant
cannot survive its root before either formal execution or semantic replay. On normal
exit, timeout, or client interruption, the supervisor kills/stops the complete unit,
resets it, confirms it is inactive, and returns a validated cleanup receipt. A formal
scientific classification becomes actionable only after matching cleanup and an
independent no-residue check. Absence of cgroup v2, the user-systemd bus, pinned
executables, exact environment, containment proof, runtime receipt, or inactive
cleanup is an infrastructure failure rather than a scientific result.
...
6. otherwise -> `MM011_inconclusive`.
...
On clean NO-GO, MM-010 remains reserved and unused. The proposed direction receives a
fresh MM-012 identity chosen from the audited residuals. The first cheap competitors
are clipped value velocity and blur-matched persistence, followed by subpixel phase
correlation or continuous ECC affine registration. A source-only analog/coverage,
cadence, operator-certification, or innovation-decomposition assay is selected only
if the complete per-family evidence identifies that failure mode.
...
On invalid or inconclusive MM-011, run neither branch. Diagnose and preregister a new
assay first. AIDE2 remains outside this one-shot assay. A bounded AIDE-style harness
comparison may be preregistered only after several cheap development evaluators and a
physically withheld one-shot judge exist; it cannot adapt on MM-011's real future
targets.

## 10. Artifacts and verification

Canonical output is
`bench/multimodal_causal_assay/results/MM-011/`. Prepared records include the
protocol/config/schema, exact source/runtime and copied NumPy/stdlib/custody manifests,
the complete copied LCV-001 package, LCV verification/binding record, pre-marker
isolation evidence, independent pre-real audit, and freeze record. Formal records add
the `0444` start, actual-child runtime receipt, and synthetic-control markers,
...
the copied LCV package and successor-surface pins, marker ordering, actual-child
runtime receipt, prediction byte counts, 453-row commit chain, primitive
...
total-wall ceiling, independently rechecks the copied LCV/runtime surface, reruns and
bit-compares the formal synthetic panel, reconstructs normalizers/rows from the copied
LCV parent without historical verifier imports, reruns each source-only predictor in canonical
...
Abandon this exact run before real scoring if the canonical or copied LCV package,
runtime commitments, or v2.2 dependency hashes drift; exact row/normalizer parity
fails; the off-grid sensitivity audit misses its frozen minimum; the
Landlock/seccomp sandbox cannot
