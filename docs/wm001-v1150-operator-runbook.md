# WM-001 v1.15.0 operator runbook

Status: prospective. Seal this runbook with the exact v1.15 protocol,
implementation, schemas, lock, tests, and independent review before any v1.15
producer path is created.

Protocols 1.10.0 through 1.14.0 are terminally retired. Versions 1.10 through
1.13 successively exposed and repaired QA/runtime composition, deprecated
precision access on stderr, empty-stream schema, and canonical-versus-preserved
preformal-report role defects.

Version 1.14 passed every prospective gate, its result-free command-10
rehearsal, development producer, independently reproduced development audit,
closure, fresh sealed-runtime reopen, and all ten preformal commands. Its sole
binding transaction then reached the independent formal-input archive auditor.
That auditor had matched and verified every one of the canonical archive's 86
declared members, but its final exhaustion check called `iter(stream)` a second
time. `TarFile` replayed a cached already-consumed member, which was falsely
reported as a physical extra member. Independent inspection established zero
missing and zero extra members. The terminal failed binding attempt was
outer-finalized; no formal-input preflight, accepted binding, formal launch,
formal producer, formal audit, semantic review, or adjudication exists.

Do not inspect or use any retained K or performance value. Nothing in this
runbook repairs, resumes, removes, or reuses v1.10 through v1.14 evidence,
environments, wheels, seals, locks, bindings, or outer completions. The exact
disposition is documented in the
[v1.14 independent archive-verifier failure](wm001-v1140-development-archive-membership-failure.md).

The active protocol must directly supersede v1.14.0 and pin its exact sealed
protocol SHA-256 as
...
QA_PY="/home/alex/.venvs/prospect-wm001-v115-reviewed/bin/python"
RUNTIME_PY="/home/alex/.venvs/prospect-wm001-v115-reviewed-runtime/bin/python"
...
DEV="$DEV_ROOT/qualification-v1.15.0"
DEV_DIAGNOSTICS="$DEV_ROOT/diagnostics-v1.15.0"
RUNTIME_SEAL="$DEV_ROOT/runtime-seal-v1.15.0.json"
DEV_CLOSURE="$DEV_ROOT/development-closure-v1.15.0.json"
PREFORMAL_ROOT="$DEV_ROOT/v1.15.0/preformal"
PREFORMAL_CLAIM="$DEV_ROOT/v1.15.0/.preformal.staging"
PREFORMAL="$PREFORMAL_ROOT/preformal-test-report-v1.15.0.json"

OPERATOR_ROOT="$REPO/bench/world_model_lifecycle/results/operator-v1.15"
DEV_AUDIT="$OPERATOR_ROOT/audits/development-audit-v1.15.0"
CLOSURE_ATTEMPT="$OPERATOR_ROOT/closures/development-closure-v1.15.0"
BINDING_ATTEMPT="$OPERATOR_ROOT/bindings/formal-binding-v1.15.0"
BINDING="$BINDING_ATTEMPT/formal-binding.json"
FORMAL_INPUT_PREFLIGHT="$BINDING_ATTEMPT/formal-input-preflight.json"

OUTER_ROOT="$REPO/bench/world_model_lifecycle/results/outer-completions/v1.15"
RUNTIME_LOCK="$REPO/bench/world_model_lifecycle/results/.wm001-v1.15-runtime.lock"
FORMAL_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-launch-v1.15.0.json"
FORMAL_AUDIT="$OPERATOR_ROOT/audits/formal-audit-v1.15.0"
FORMAL_AUDIT_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-audit-v1.15.0.json"
ADJUDICATION_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-adjudication-v1.15.0.json"
ADJUDICATION_ROOT="$REPO/bench/world_model_lifecycle/results/adjudication-v1.15"
ADJUDICATION_PACKAGE="$ADJUDICATION_ROOT/formal-adjudication-v1.15.0"

REVIEW="$REPO/docs/wm001-v1150-prospective-harness-review.json"
SEMANTIC_REVIEW="$REPO/artifacts/wm001-reviews/formal-v1.15.0.json"
...
require_v115_lifecycle_absent() {
  ...
  if find "$REPO/bench/world_model_lifecycle/results/formal" \
      -mindepth 1 -name '*v1.15.0*' -print -quit 2>/dev/null |
      grep -q .
  then
    echo "unexpected v1.15 formal lifecycle path exists" >&2
    return 2
  fi
}
```

The development masters are exactly `2388891654, 3201418215`. The formal
masters are exactly `2465968807, 3494485289, 1615601571, 2220840580,
280448223, 597199725, 712207456, 1727907751`. The verifier must verify 140
unique prior masters and 19,040 unique prior streams through v1.14 before any
evidence run.
...
WHEELHOUSE="$(mktemp -d /tmp/prospect-wm001-v115-wheelhouse.XXXXXX)"
...
generated WM-001 header must name v1.15.0. The Prospect row must equal the
...
lines = [
    "# WM-001 protocol 1.15.0 isolated live execution closure.",
    "# Versions preserve the v1.4 scientific runtime; each v1.15 digest covers",
...
```bash
REVIEW_DRAFT="$REPO/docs/wm001-v1150-prospective-harness-review.draft.json"
test -f "$REVIEW_DRAFT"
...
- the canonical verifier accepts only the original v1.15 development bundle
...
- the preserved verifier rejects a missing or changed referenced stream and
  any additional report or `preformal-v1.15.0-command-*` member; and
...
- direct supersession of v1.14.0 protocol SHA-256
  `39f5820a91c8a504355f971449726ae0a9067cc856111a575bb038455d1fd635`;
- scientific-block SHA-256
  `fa44fd93a672db3905d45a0e99c568985e7e2e5d02d32043c830db413005a5c3`;
- 10 current masters, 1,360 current streams, 140 prior masters, and 19,040
...
- unchanged raw-result v9 and formal-binding v10 representation identities,
  with only the v1.15 protocol and formal-seed constants updated; and
...
```bash
COMMAND10_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v115-command10.XXXXXX)"
...
if [ "$COMMAND10_RC" -ne 0 ] || [ -s "$COMMAND10_STDERR" ]; then
  echo "result-free command 10 failed or wrote stderr; v1.15 is retired" >&2
...
stderr retires v1.15; do not repair its source, wheel, environment, seal, or
capture in place.
...
v1.15 producer root. Verify the exact real-subprocess branch tests explicitly
...
nonzero return, or nonempty stderr consumes and retires v1.15; do not repeat
...
the v1.15 development qualification even if any later phase fails.
...
or missing ordinary audit retires v1.15; never rerun it.
...
binding, or stop/go failure retires v1.15. Absence of a downstream path never
...
`$DEV_ROOT/v1.15.0/.preformal.staging`, then fsyncs its parent before its first
...
require_absent "$DEV_ROOT/v1.15.0/.preformal.staging"
...
either path or retry v1.15. A successful generation is not sufficient by
...
version `1.15.0`, `passed: true`, and digests for the binding, preformal report,
development closure, accepted-closure evidence, runtime conformance, and
auditor source. Any missing, false, stale, or mismatched receipt retires v1.15.
...
FORMAL="$FORMAL_BINDING_ROOT/confirmation-v1.15.0"
...
sibling left by an earlier launcher, contaminates and consumes the v1.15
...
if [ ! -d "$FORMAL_AUDIT" ]; then
  echo "formal audit failed before publishing an authenticated attempt; v1.15 is retired" >&2
  exit 2
fi
"$QA_PY" -I -B -c \
...
establishes that the sole v1.15 formal-audit claim was consumed, even if no
...
bounded WM-001 claim. Every other terminal state retires v1.15 without
