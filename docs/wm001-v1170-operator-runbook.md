# WM-001 v1.17.0 operator runbook

Status: prospective. Seal this runbook with the exact v1.17 protocol,
implementation, schemas, lock, tests, and independent review before any v1.17
producer path is created.

Two unbound environment preparations were discarded before prospective review
or lifecycle-path creation. The first exposed a case-sensitive launch-rule
guard mismatch in the full protocol verifier. The second exposed that the
producer-side preformal type-check list omitted the new rehearsal module while
the independent auditor and runbook included it. The guard now checks the
sealed sentence exactly, a direct regression executes the complete verifier,
and both preformal implementations require the same rehearsal type check. This
runbook exclusively uses fresh `-r3` environment paths. The discarded
environments are not evidence and cannot authorize this attempt.

Protocols 1.10.0 through 1.16.0 are terminally retired. Version 1.16 passed
every prospective, development, audit, closure, preformal, binding, preflight,
and final stop/go gate. Its mandatory accepted-binding pre-root rehearsal then
failed before child dispatch, formal-root creation, or any formal outcome.

The production reproduction receipt and operator terminal named the canonical
content-addressed runtime sidecar, while the standard-library launcher required
the distinct second-execution capture filename. A read-only counterfactual
confirmed that this was the sole pre-dispatch mismatch, but the repair must
remove the duplicated contract and synthetic fixture rather than replace one
literal. Do not inspect or use any retained K or performance value. Nothing in
this runbook repairs, resumes, removes, or reuses v1.10 through v1.16 evidence,
environments, wheels, seals, locks, bindings, outer completions, or diagnostics.
The exact disposition is documented in the
[v1.16 rehearsal failure](wm001-v1160-accepted-binding-rehearsal-failure.md).

The active protocol must directly supersede v1.16.0 and pin its exact sealed
protocol SHA-256 as
...
QA_PY="/home/alex/.venvs/prospect-wm001-v117-reviewed-r3/bin/python"
RUNTIME_PY="/home/alex/.venvs/prospect-wm001-v117-reviewed-runtime-r3/bin/python"
...
DEV="$DEV_ROOT/qualification-v1.17.0"
DEV_DIAGNOSTICS="$DEV_ROOT/diagnostics-v1.17.0"
RUNTIME_SEAL="$DEV_ROOT/runtime-seal-v1.17.0.json"
DEV_CLOSURE="$DEV_ROOT/development-closure-v1.17.0.json"
PREFORMAL_ROOT="$DEV_ROOT/v1.17.0/preformal"
PREFORMAL_CLAIM="$DEV_ROOT/v1.17.0/.preformal.staging"
PREFORMAL="$PREFORMAL_ROOT/preformal-test-report-v1.17.0.json"

OPERATOR_ROOT="$REPO/bench/world_model_lifecycle/results/operator-v1.17"
DEV_AUDIT="$OPERATOR_ROOT/audits/development-audit-v1.17.0"
CLOSURE_ATTEMPT="$OPERATOR_ROOT/closures/development-closure-v1.17.0"
BINDING_ATTEMPT="$OPERATOR_ROOT/bindings/formal-binding-v1.17.0"
BINDING="$BINDING_ATTEMPT/formal-binding.json"
FORMAL_INPUT_PREFLIGHT="$BINDING_ATTEMPT/formal-input-preflight.json"
REHEARSAL_ATTEMPT="$OPERATOR_ROOT/rehearsals/accepted-binding-rehearsal-v1.17.0"
REHEARSAL_CLAIM_ROOT="$REPO/bench/world_model_lifecycle/results/rehearsals/v1.17"

OUTER_ROOT="$REPO/bench/world_model_lifecycle/results/outer-completions/v1.17"
RUNTIME_LOCK="$REPO/bench/world_model_lifecycle/results/.wm001-v1.17-runtime.lock"
FORMAL_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-launch-v1.17.0.json"
FORMAL_AUDIT="$OPERATOR_ROOT/audits/formal-audit-v1.17.0"
FORMAL_AUDIT_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-audit-v1.17.0.json"
ADJUDICATION_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-adjudication-v1.17.0.json"
ADJUDICATION_ROOT="$REPO/bench/world_model_lifecycle/results/adjudication-v1.17"
ADJUDICATION_PACKAGE="$ADJUDICATION_ROOT/formal-adjudication-v1.17.0"

REVIEW="$REPO/docs/wm001-v1170-prospective-harness-review.json"
SEMANTIC_REVIEW="$REPO/artifacts/wm001-reviews/formal-v1.17.0.json"
...
require_v117_lifecycle_absent() {
  ...
  if find "$REPO/bench/world_model_lifecycle/results/formal" \
      -mindepth 1 -name '*v1.17.0*' -print -quit 2>/dev/null |
      grep -q .
  then
    echo "unexpected v1.17 formal lifecycle path exists" >&2
    return 2
  fi
}
```

The development masters are exactly `3454397035, 2131905789`. The formal
masters are exactly `3651766805, 1960341898, 785042759, 1752824577,
3284431163, 2694043685, 2970882769, 386448916`. The verifier must verify 160
unique prior masters and 21,760 unique prior streams through v1.16 before any
...
WHEELHOUSE="$(mktemp -d /tmp/prospect-wm001-v117-wheelhouse.XXXXXX)"
...
generated WM-001 header must name v1.17.0. The Prospect row must equal the
...
lines = [
    "# WM-001 protocol 1.17.0 isolated live execution closure.",
    "# Versions preserve the v1.4 scientific runtime; each v1.17 digest covers",
...
```bash
REVIEW_DRAFT="$REPO/docs/wm001-v1170-prospective-harness-review.draft.json"
test -f "$REVIEW_DRAFT"
...
- the canonical verifier accepts only the original v1.17 development bundle
...
- the preserved verifier rejects a missing or changed referenced stream and
  any additional report or `preformal-v1.17.0-command-*` member; and
...
- direct supersession of v1.16.0 protocol SHA-256
  `ac7a8aa331f15412c80a1dad6af9b30c154db33b6d313940e8d2ee546b57dc00`;
- scientific-block SHA-256
  `fa44fd93a672db3905d45a0e99c568985e7e2e5d02d32043c830db413005a5c3`;
- 10 current masters, 1,360 current streams, 160 prior masters, and 21,760
...
- unchanged raw-result v9 and formal-binding v10 representation identities,
  with only the v1.17 protocol and formal-seed constants updated; and
...
```bash
COMMAND10_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v117-command10.XXXXXX)"
...
if [ "$COMMAND10_RC" -ne 0 ] || [ -s "$COMMAND10_STDERR" ]; then
  echo "result-free command 10 failed or wrote stderr; v1.17 is retired" >&2
...
stderr retires v1.17; do not repair its source, wheel, environment, seal, or
capture in place.
...
v1.17 producer root. Verify the exact real-subprocess branch tests explicitly
...
nonzero return, or nonempty stderr consumes and retires v1.17; do not repeat
...
the v1.17 development qualification even if any later phase fails.
...
or missing ordinary audit retires v1.17; never rerun it.
...
binding, or stop/go failure retires v1.17. Absence of a downstream path never
...
`$DEV_ROOT/v1.17.0/.preformal.staging`, then fsyncs its parent before its first
...
require_absent "$DEV_ROOT/v1.17.0/.preformal.staging"
...
either path or retry v1.17. A successful generation is not sufficient by
...
version `1.17.0`, `passed: true`, and digests for the binding, preformal report,
development closure, accepted-closure evidence, runtime conformance, and
auditor source. Any missing, false, stale, or mismatched receipt retires v1.17.
...
FORMAL="$FORMAL_BINDING_ROOT/confirmation-v1.17.0"
...
REHEARSAL_OUTER_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v117-rehearsal-operator.XXXXXX)"
...
then
  REHEARSAL_RECOVERY_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v117-rehearsal-recovery.XXXXXX)"
  set +e
  "${SAFE_ENV[@]}" "$RUNTIME_PY" -I -S -B "$LAUNCH" \
    --bootstrap "$BOOTSTRAP" \
    --rehearse-accepted-binding "$BINDING" \
...
unfinalized evidence—or any created formal path—retires v1.17. Recovery can
...
# and every alternate/current v1.17 formal, audit, review, and adjudication
...
sibling left by an earlier launcher, contaminates and consumes the v1.17
...
before any outcome reset and permanently consumes v1.17.
...
if [ ! -d "$FORMAL_AUDIT" ]; then
  echo "formal audit failed before publishing an authenticated attempt; v1.17 is retired" >&2
  exit 2
fi
"$QA_PY" -I -B -c \
...
establishes that the sole v1.17 formal-audit claim was consumed, even if no
...
bounded WM-001 claim. Every other terminal state retires v1.17 without
