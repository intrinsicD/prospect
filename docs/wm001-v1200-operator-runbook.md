# WM-001 v1.20.0 operator runbook

Status: prospective. Seal this runbook with the exact v1.20 protocol,
implementation, schemas, lock, tests, and independent review before any v1.20
producer path is created.

Protocols 1.10.0 through 1.19.0 are terminally retired. Version 1.19 passed its
prospective gates, completed the sole development producer, and completed the
full outcome audit twice with byte-identical output and passing capacity
evidence. Its sole closure then rejected the valid two-link outer-finalized
producer manifest through a generic reader's hidden one-link default. The
canonical closure marker was never published and no preformal, binding, or
formal authority exists. This was not evidence against the scientific claim;
it exposed a custody-composition defect.
...
through v1.19 evidence, environments, wheels, seals, locks, bindings, outer
...
The active protocol must directly supersede v1.19.0 and pin its exact sealed
protocol SHA-256 as
...
QA_PY="/home/alex/.venvs/prospect-wm001-v120-reviewed/bin/python"
RUNTIME_PY="/home/alex/.venvs/prospect-wm001-v120-reviewed-runtime/bin/python"
...
DEV="$DEV_ROOT/qualification-v1.20.0"
DEV_DIAGNOSTICS="$DEV_ROOT/diagnostics-v1.20.0"
RUNTIME_SEAL="$DEV_ROOT/runtime-seal-v1.20.0.json"
DEV_CLOSURE="$DEV_ROOT/development-closure-v1.20.0.json"
PREFORMAL_ROOT="$DEV_ROOT/v1.20.0/preformal"
PREFORMAL_CLAIM="$DEV_ROOT/v1.20.0/.preformal.staging"
PREFORMAL="$PREFORMAL_ROOT/preformal-test-report-v1.20.0.json"

OPERATOR_ROOT="$REPO/bench/world_model_lifecycle/results/operator-v1.20"
DEV_AUDIT="$OPERATOR_ROOT/audits/development-audit-v1.20.0"
CLOSURE_ATTEMPT="$OPERATOR_ROOT/closures/development-closure-v1.20.0"
BINDING_ATTEMPT="$OPERATOR_ROOT/bindings/formal-binding-v1.20.0"
BINDING="$BINDING_ATTEMPT/formal-binding.json"
FORMAL_INPUT_PREFLIGHT="$BINDING_ATTEMPT/formal-input-preflight.json"
REHEARSAL_ATTEMPT="$OPERATOR_ROOT/rehearsals/accepted-binding-rehearsal-v1.20.0"
REHEARSAL_CLAIM_ROOT="$REPO/bench/world_model_lifecycle/results/rehearsals/v1.20"

OUTER_ROOT="$REPO/bench/world_model_lifecycle/results/outer-completions/v1.20"
RUNTIME_LOCK="$REPO/bench/world_model_lifecycle/results/.wm001-v1.20-runtime.lock"
FORMAL_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-launch-v1.20.0.json"
FORMAL_AUDIT="$OPERATOR_ROOT/audits/formal-audit-v1.20.0"
FORMAL_AUDIT_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-audit-v1.20.0.json"
ADJUDICATION_MARKER="$REPO/bench/world_model_lifecycle/results/formal/formal-adjudication-v1.20.0.json"
ADJUDICATION_ROOT="$REPO/bench/world_model_lifecycle/results/adjudication-v1.20"
ADJUDICATION_PACKAGE="$ADJUDICATION_ROOT/formal-adjudication-v1.20.0"

REVIEW="$REPO/docs/wm001-v1200-prospective-harness-review.json"
SEMANTIC_REVIEW="$REPO/artifacts/wm001-reviews/formal-v1.20.0.json"
...
require_v120_lifecycle_absent() {
  ...
  if find "$REPO/bench/world_model_lifecycle/results/formal" \
      -mindepth 1 -name '*v1.20.0*' -print -quit 2>/dev/null |
      grep -q .
  then
    echo "unexpected v1.20 formal lifecycle path exists" >&2
    return 2
  fi
}
```

The development masters are exactly `3626676950, 2572962267`. The formal
masters are exactly `3772418031, 1586188972, 155797552, 2704051827,
818738828, 4077496645, 1566512625, 2151461680`. The verifier must verify 190
unique prior masters and 25,840 unique prior streams through v1.19 before any
...
The closure custody repair has its own mandatory composition gate. Every
binding stable-file reader must require an explicit link-count argument. The
real `create_development_closure()` chain must accept the outer-finalized
two-link manifest, derive its object and digest from the same captured bytes,
and avoid `sha256_file()` for that manifest. The same creator chain must reject
one-link, three-link, symlinked, in-read-mutated, and public-verifier-divergent
manifests before downstream evidence validation.
...
WHEELHOUSE="$(mktemp -d /tmp/prospect-wm001-v120-wheelhouse.XXXXXX)"
...
generated WM-001 header must name v1.20.0. The Prospect row must equal the
...
lines = [
    "# WM-001 protocol 1.20.0 isolated live execution closure.",
    "# Versions preserve the v1.4 scientific runtime; each v1.20 digest covers",
...
```bash
REVIEW_DRAFT="$REPO/docs/wm001-v1200-prospective-harness-review.draft.json"
test -f "$REVIEW_DRAFT"
...
"$QA_PY" -I -B -m pytest -q \
  ...
  tests/test_world_model_binding.py::test_binding_stable_file_readers_require_explicit_link_custody \
  tests/test_world_model_binding.py::test_outer_finalized_manifest_capture_binds_exact_object_and_digest \
  tests/test_world_model_binding.py::test_outer_finalized_manifest_capture_requires_exactly_two_links \
  tests/test_world_model_binding.py::test_development_closure_captures_outer_finalized_manifest_for_object_and_digest \
  tests/test_world_model_binding.py::test_development_closure_rejects_nonfinal_manifest_link_custody \
  tests/test_world_model_binding.py::test_development_closure_rejects_manifest_mutation_during_exact_capture \
  tests/test_world_model_binding.py::test_development_closure_rejects_symlinked_manifest_in_creator_chain \
  tests/test_world_model_binding.py::test_development_closure_never_uses_loose_manifest_digest_read \
  tests/test_world_model_binding.py::test_development_closure_rejects_manifest_verifier_divergence \
...
- the canonical verifier accepts only the original v1.20 development bundle
...
- the preserved verifier rejects a missing or changed referenced stream and
  any additional report or `preformal-v1.20.0-command-*` member; and
...
- direct supersession of v1.19.0 protocol SHA-256
  `07c6fe364aeddbd5689fa4f638a6f9a38506b16e8845a947fffa87e01eb3854a`;
- scientific-block SHA-256
  `fa44fd93a672db3905d45a0e99c568985e7e2e5d02d32043c830db413005a5c3`;
- 10 current masters, 1,360 current streams, 190 prior masters, and 25,840
...
- unchanged raw-result v9 and formal-binding v10 representation identities,
  with their version-scoped seed and path identities advanced to v1.20;
...
```bash
COMMAND10_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v120-command10.XXXXXX)"
...
if [ "$COMMAND10_RC" -ne 0 ] || [ -s "$COMMAND10_STDERR" ]; then
  echo "result-free command 10 failed or wrote stderr; v1.20 is retired" >&2
...
stderr retires v1.20; do not repair its source, wheel, environment, seal, or
capture in place.
...
v1.20 producer root. Verify the exact real-subprocess branch tests explicitly
...
nonzero return, or nonempty stderr consumes and retires v1.20; do not repeat
...
the v1.20 development qualification even if any later phase fails.
...
binding, or stop/go failure retires v1.20. Absence of a downstream path never
...
`$DEV_ROOT/v1.20.0/.preformal.staging`, then fsyncs its parent before its first
...
require_absent "$DEV_ROOT/v1.20.0/.preformal.staging"
...
either path or retry v1.20. A successful generation is not sufficient by
...
version `1.20.0`, `passed: true`, and digests for the binding, preformal report,
development closure, accepted-closure evidence, runtime conformance, and
auditor source. Any missing, false, stale, or mismatched receipt retires v1.20.
...
FORMAL="$FORMAL_BINDING_ROOT/confirmation-v1.20.0"
...
REHEARSAL_OUTER_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v120-rehearsal-operator.XXXXXX)"
...
then
  REHEARSAL_RECOVERY_CAPTURE="$(mktemp -d /tmp/prospect-wm001-v120-rehearsal-recovery.XXXXXX)"
  set +e
  "${SAFE_ENV[@]}" "$RUNTIME_PY" -I -S -B "$LAUNCH" \
    --bootstrap "$BOOTSTRAP" \
    --rehearse-accepted-binding "$BINDING" \
...
unfinalized evidence—or any created formal path—retires v1.20. Recovery can
...
# and every alternate/current v1.20 formal, audit, review, and adjudication
...
sibling left by an earlier launcher, contaminates and consumes the v1.20
...
before any outcome reset and permanently consumes v1.20.
...
if [ ! -d "$FORMAL_AUDIT" ]; then
  echo "formal audit failed before publishing an authenticated attempt; v1.20 is retired" >&2
  exit 2
fi
"$QA_PY" -I -B -c \
...
establishes that the sole v1.20 formal-audit claim was consumed, even if no
...
bounded WM-001 claim. Every other terminal state retires v1.20 without
