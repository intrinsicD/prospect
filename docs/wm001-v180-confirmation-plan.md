# WM-001 protocol 1.8.0 confirmation plan

Status: prospective. This document must be sealed with the v1.8 protocol,
schemas, implementation, dependency lock, tests, and independent harness review
before any v1.8 development or formal outcome is produced.

Protocol 1.7.0 is immutable and retired. Its sole development producer
completed, its canonical development audit was accepted, and the sole
development-closure attempt terminated as outer-finalized failure evidence.
The closure's final live producer check tried to materialize the 320,556,697
byte authenticated result through a helper capped at 64 MiB. There is no v1.7
development closure, preformal authorization, binding, or formal launch. No
v1.7 K3–K6 value may be inspected, summarized, compared, or used to select any
v1.8 scientific or engineering setting.

## Purpose and scientific freeze

WM-001 v1.8.0 permits one fresh-seed confirmation of the unchanged
collect → learn → improve → retain → restart experiment. The newly authorized
repairs replace the closure's size-limited whole-file result read with bounded
descriptor-streamed hashing and stable namespace custody, require an exact
byte-canonical qualification archive and strict JSON scalar types, retain only
a bounded authenticated terminal diagnostic identity, and make every
independent reader require the one canonical formal output directory.

The repairs may change result-file and archive custody, failure diagnostics,
formal-path authorization, prospective conformance, tests, versioned paths,
...
- every v1.8 protocol, verifier, auditor, prospective review, and result schema
  agrees on the new version, paths, seeds, and exact support-file set.

The v1.7 closure failure is engineering lineage only. It supplies no evidence
for or against the scientific claim.

## Assurance boundary

Every v1.8 seal, binding, operator record, audit receipt, and adjudication
...
```text
SHA256("WM-001|1.8.0|<lane>-master|<i>")
```

Development indices 0–1 are:

```text
1196068124, 758859051
```
...
- 70 unique exposed prior masters and 9,520 unique exposed prior streams from
  v1.0.0, v1.2.0, v1.3.0, v1.4.0, v1.5.0, v1.6.0, and v1.7.0; and
- zero overlap in all current/prior master/stream cross classes.

No seed is replaced after any v1.8 outcome-producing path is created.
...
There is exactly one v1.8 development producer:

```text
bench/world_model_lifecycle/results/development/qualification-v1.8.0
```

Exclusive creation of that root consumes the v1.8 development qualification.
...
```text
bench/world_model_lifecycle/results/operator-v1.8/audits/development-audit-v1.8.0
bench/world_model_lifecycle/results/operator-v1.8/closures/development-closure-v1.8.0
bench/world_model_lifecycle/results/development/development-closure-v1.8.0.json
```
...
v1.8. A terminal failure does not restore authorization merely because a later
...
report runs the exact ten command roles inherited from v1.7: eight isolated QA
...
```text
bench/world_model_lifecycle/results/operator-v1.8/bindings/formal-binding-v1.8.0
```
...
```text
bench/world_model_lifecycle/results/formal/formal-launch-v1.8.0.json
```
