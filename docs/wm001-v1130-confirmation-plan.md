# WM-001 protocol 1.13.0 confirmation plan

Status: prospective. This document must be sealed with the v1.13 protocol,
schemas, implementation, dependency lock, tests, and independent harness review
before any v1.13 development or formal outcome is produced.

Protocols 1.10.0, 1.11.0, and 1.12.0 are immutable and terminally retired.
Version 1.10 exposed a cross-environment QA/runtime composition error at
preformal. Version 1.11 repaired that class but its result-free command-10
rehearsal exposed deprecated TF32 access by writing a warning to stderr.
Version 1.12 replaced the legacy precision APIs, enforced nested zero-stderr
custody, passed its result-free rehearsal, completed and outer-finalized
development, an accepted independent audit, closure, and all ten preformal
commands, then failed its sole binding transaction.

The v1.12 binding producer correctly retained ten required zero-byte stderr
logs. Its formal-binding JSON Schema incorrectly reused a nonempty
implementation-file digest definition for those stream identities, so the
first `source.test_log_files[*].bytes: 0` failed the schema's `minimum: 1`.
The failed binding attempt was outer-finalized; no formal launch or formal
outcome occurred. Precommitted automated verification and custody processed the
claim-ineligible development result, but no v1.12 K or performance value was
manually opened, printed, summarized, compared, thresholded, or used to select
a v1.13 setting. All v1.10–v1.12 evidence remains claim-ineligible and cannot
authorize, block, resume, or be reused by v1.13.
The immediate disposition is preserved in the
[v1.12 binding-schema failure](wm001-v1120-binding-schema-failure.md).

## Purpose and scientific freeze

WM-001 v1.13.0 permits one fresh-seed confirmation of the unchanged
...
It changes only the evidence representation and prospective coverage needed to
repair that contradiction:

1. formal-binding representation v10 keeps `fileDigest.bytes >= 1` for
   implementation files and introduces a separate stream-file digest with
   `bytes >= 0` exclusively for the 20 preformal stdout/stderr identities;
2. raw-result representation v9, prebinding request/report v2, preformal
   report v2, command-9 closure semantics, command-10 exact-zero-stderr
   semantics, precision custody, and recorded-versus-live runtime boundaries
   remain unchanged;
3. a producer-to-schema compatibility preflight validates a complete
   real-shaped binding candidate containing ten nonempty stdout and ten empty
   stderr rows before any one-shot evidence path exists;
4. adversarial tests accept zero-byte stream identities, reject negative
   lengths and wrong empty-stream digests, and continue to reject empty
   implementation files and nonempty command-10 stderr;
5. the central verifier and independent auditor add v1.12 to the prior
   master/stream universe and reproduce all cross-domain collision counts; and
6. every active namespace, master seed, environment, wheel, lock, seal,
...
The v1.13 repair may change only the formal-binding log-file representation,
schema/producer compatibility gates, versioned paths, fresh seeds, and the
evidence plumbing needed to bind those fields. It may not change the world
model, learning algorithm, optimizer, planner, controller, task definitions,
...
- every v1.13 protocol, verifier, auditor, prospective review, and result schema
  agrees on the new version, paths, seeds, and exact support-file set.

The v1.10, v1.11, and v1.12 failures are engineering lineage only. They supply
no evidence for or against the scientific claim.

## Assurance boundary

Every v1.13 seal, binding, operator record, audit receipt, and adjudication
...
```text
SHA256("WM-001|1.13.0|<lane>-master|<i>")
```

Development indices 0–1 are:

```text
560818116, 1392377688
```
...
- 120 unique exposed prior masters and 16,320 unique exposed prior streams from
  v1.0.0, v1.2.0, v1.3.0, v1.4.0, v1.5.0, v1.6.0, v1.7.0, v1.8.0,
  v1.9.0, v1.10.0, v1.11.0, and v1.12.0; and
- zero overlap in all current/prior master/stream cross classes.

No seed is replaced after any v1.13 outcome-producing path is created.

## Exact runtime and environment

The QA and producer environments are fresh, separate, non-editable virtual
environments at
`/home/alex/.venvs/prospect-wm001-v113-reviewed` and
`/home/alex/.venvs/prospect-wm001-v113-reviewed-runtime`. Both install the one
reviewed wheel built in a fresh `prospect-wm001-v113-wheelhouse.*` directory.
...
There is exactly one v1.13 development producer:

```text
bench/world_model_lifecycle/results/development/qualification-v1.13.0
```

Exclusive creation of that root consumes the v1.13 development qualification.
...
`results/development/diagnostics-v1.13.0`; they cannot occupy any lifecycle
namespace. The sole canonical development audit and closure paths are:

```text
bench/world_model_lifecycle/results/operator-v1.13/audits/development-audit-v1.13.0
bench/world_model_lifecycle/results/operator-v1.13/closures/development-closure-v1.13.0
bench/world_model_lifecycle/results/development/development-closure-v1.13.0.json
```
...
failure—retires v1.13. A terminal failure does not restore authorization merely
...
```text
bench/world_model_lifecycle/results/development/v1.13.0/preformal
```
...
the hidden claim; it still consumes v1.13 and cannot be deleted and retried.
...
```text
bench/world_model_lifecycle/results/operator-v1.13/bindings/formal-binding-v1.13.0
```
...
```text
bench/world_model_lifecycle/results/formal/formal-launch-v1.13.0.json
```
