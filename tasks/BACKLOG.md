...
- **P14-001** · `demonstrated (non-gated, swingup)` · R5,R7 · Observe → repeat: reproduce a demonstrated behavior the agent never performed itself (imitation-from-observation). **Shown on cartpole swingup** (BH-001 §B, ADR-0012): watch an expert's *observations only* → recover its actions (direct inverse-dynamics primary; P13 latent-action route arc-faithful but high-variance) → clone a closed-loop policy → reproduce. Inverse-dyn imitation **45.3 vs from-scratch 6.4** (7×), shuffled control 0.1 — watching does what exploration (A) can't at the same budget; then **explore** (P3-002) fills what watching can't teach. *Numpy kill-gate on a toy = follow-up to ship P14 in the ratchet.* Runtime layer on top: real-YouTube ingestion + live-webcam demo (non-gated).
