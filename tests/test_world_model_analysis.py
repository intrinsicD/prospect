...
def test_scientific_revision_uses_fresh_v1200_seed_domain() -> None:
    master = 3_626_676_950
    assert derive_seed("model_initialization", master, 0) == 3_554_237_159
    assert derive_seed("planner", master, 0) == 3_182_258_505
    assert derive_seed("collection_action", master, 1) == 738_900_297
    assert derive_seed("irrelevant_collection_action", master, 0) == 615_670_841
    assert derive_seed("collect_irrelevant_episode", master, 0) == 3_469_312_547
    assert derive_seed("predictive_validation_irrelevant_action", master, 0) == 2_159_724_402
    assert derive_seed("predictive_validation_irrelevant_episode", master, 0) == 1_373_322_386


def _seed(namespace: str, master: int, index: int) -> int:
    payload = f"WM-001|1.20.0|{namespace}|{master}|{index}".encode()
