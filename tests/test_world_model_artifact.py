...
def test_current_protocol_passes_complete_static_verifier() -> None:
    ...
    assert protocol["experiment"]["protocol_version"] == "1.18.0"
...
def test_protocol_wide_formal_launch_claim_is_atomic_across_bindings(
    ...
    assert copied["protocol_version"] == "1.18.0"
...
def test_formal_launch_claim_rejects_noncanonical_output(tmp_path: Path) -> None:
    ...
    for output in invalid_outputs:
        with pytest.raises(
            ValueError,
            match="results/formal/<binding-sha256>/confirmation-v1.18.0",
        ):
            formal_launch_marker_path(
                binding,
                output,
...
def test_formal_cli_preclaim_failures_leave_v118_marker_absent(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    failure_stage: str,
) -> None:
