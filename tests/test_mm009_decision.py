...
from bench.multimodal_causal_diagnostics import decision, scoring
...
def test_intermediate_or_future_only_support_is_inconclusive(
    ...
    summary = decision.derive_decision(_evidence(modes, overrides=overrides))
    assert summary.decision == "MM009_inconclusive"
...
@pytest.mark.parametrize(
    ("count", "expected"),
    [
        (6, "invalid_MM009_real_negative_control"),
        (4, "MM009_inconclusive"),
    ],
)
...
def test_fold_activity_and_range_each_preempt_go() -> None:
    ...
    assert fold_summary.decision == "MM009_inconclusive"
    ...
    assert activity_summary.decision == "MM009_inconclusive"
    ...
    assert range_summary.decision == "MM009_inconclusive"
...
def test_invalid_prerequisite_precedes_real_null_and_incomplete_is_invalid() -> None:
    ...
    assert summary.decision == "invalid_MM009"
    ...
    assert incomplete.decision == "invalid_MM009"
...
def test_shared_primitives_schema_and_claim_tampering_fail_closed() -> None:
    ...
    forged = replace(summary, decision="MM009_inconclusive")
