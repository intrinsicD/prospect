...
def test_independent_preformal_log_parser_requires_empty_stderr() -> None:
    ...
    empty = {
        "file": (
            "preformal-v1.20.0-command-02-ruff.stderr."
            f"{empty_digest}.log"
        ),
        "bytes": 0,
        "sha256": empty_digest,
...
def _write_minimal_auditable_artifact(root: Path) -> Path:
    ...
    parameter_sha256 = runtime.digest
    model_version = runtime.version
    master_seed = 3626676950

    def seed(namespace: str, index: int = 0) -> int:
        return int.from_bytes(
            hashlib.sha256(f"WM-001|1.20.0|{namespace}|{master_seed}|{index}".encode()).digest()[:4],
            "big",
        )
