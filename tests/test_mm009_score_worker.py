...
from bench.multimodal_causal_diagnostics import preparation, records, runtime, score_worker, scoring
...
def _worker_evidence(
    ...
    *,
    ...
    for index, role in enumerate(score_worker.PREDICTION_ROLES):
        mm009 = records.scientific_array_sha256(
    ...
    evidence: dict[str, records.JsonValue] = {
        ...
        "schema_version": "mm009-source-worker-v1",
...
def test_score_launcher_sandbox_denies_sibling_live_code_network_and_process_escape(
    ...
    live_predictor = Path(__file__).resolve().parents[1] / "bench/multimodal_causal_diagnostics/predictor.py"
    malicious_worker = custody_root / "bench/multimodal_causal_diagnostics/score_worker.py"
    malicious_source = f"""\
...
def test_score_worker_import_graph_and_fresh_process_exclude_fitting_modules(tmp_path: Path) -> None:
    ...
    code = (
        "import sys\n"
        "from bench.multimodal_causal_diagnostics import score_worker\n"
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "bench.multimodal_causal_diagnostics.score_worker",
            str(fixture["source"]),
            str(fixture["target"]),
            str(fixture["prediction_path"]),
            str(fixture["commit_path"]),
