...
from bench.multimodal_causal_assay import (
    experiment,
    future_isolation,
...
from bench.sealed_lineage_verifier import custody as lcv_custody
...
def test_scientific_source_census_is_bound_and_terminal_candidate_cannot_mint_go_audit() -> None:
    paths = experiment._scientific_source_paths()  # noqa: SLF001 - lifecycle contract test
    assert paths == tuple(sorted(set(paths), key=str))
    assert Path("bench/multimodal_causal_assay/experiment.py") in paths
    assert Path("bench/multimodal_causal_assay/score_worker.py") in paths
    assert Path("bench/multimodal_causal_assay/future_isolation.py") in paths
    ...
    assert set(experiment.LCV_SOURCE_DEPENDENCIES).issubset(paths)
    assert Path("bench/multimodal_causal_assay/lcv_parent.py") in paths
    ...
    # The preregistered off-grid gate terminally abandoned this finite-grid
    # candidate.  The protocol now contains that frozen gate while the retired
    # scientific modules retain their pre-gate candidate hash, so no GO audit or
    # canonical configuration can accidentally be minted from this tree.
    with pytest.raises(experiment.InvalidMM011Package, match="modules bind different protocol bytes"):
        experiment.audit_template()
...
def test_opaque_parent_copy_normalizes_only_modes_and_rejects_drift(
    ...
    monkeypatch.setattr(
        ...
        "EXPECTED_FILES",
        {
            lcv_parent.PARENT_RELATIVE / name: lcv_custody.ExpectedFile(
                sha256=pin.sha256,
    ...
    validated = experiment._validate_opaque_parent_copy(copied)  # noqa: SLF001
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="opaque immutable parent copy differs"):
        experiment._validate_opaque_parent_copy(copied)  # noqa: SLF001
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="membership differs"):
        experiment._validate_opaque_parent_copy(copied)  # noqa: SLF001
...
def test_prediction_chain_replays_canonical_worker_bundle_and_rejects_extra_entry(
    ...
    output = tmp_path / "MM-011-fixture"
    ...
    evidence: records.JsonValue = {
        "prediction_hashes": {
            role: {
                "mm011_sha256": records.scientific_array_sha256(
...
def test_formal_start_precedes_controls_and_controls_precede_parent_access(
    ...
    monkeypatch.setattr(experiment, "_validate_actual_runtime_receipt", lambda _value, *, role: {})
...
def test_formal_rejects_validator_valid_control_replacement_before_parent_access(
    ...
    monkeypatch.setattr(experiment, "_validate_actual_runtime_receipt", lambda _value, *, role: {})
    with pytest.raises(
        experiment.InvalidMM011Package,
...
def test_output_path_is_exact_and_rejects_a_symlink_alias(
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="output must be"):
        experiment._assert_output(tmp_path / "different")  # noqa: SLF001

    alias = tmp_path / "alias"
    alias.symlink_to(expected, target_is_directory=True)
    with pytest.raises(experiment.InvalidMM011Package, match="contains a symlink"):
...
def test_total_wall_deadline_is_monotonic_and_fail_closed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="total-wall deadline exceeded: fixture"):
...
def test_in_process_lifecycle_bodies_reject_cgroup_bypass(
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="cgroup"):
        experiment._run_formal_in_process(tmp_path, {})  # noqa: SLF001
    with pytest.raises(experiment.InvalidMM011Package, match="cgroup"):
...
def test_local_process_supervision_cancels_before_waiting_for_child_timeout(tmp_path: Path) -> None:
    ...
    cancel_event.set()
    with pytest.raises(experiment.InvalidMM011Package, match="cancelled after sibling failure"):
...
def test_semantic_batch_keyboard_interrupt_cancels_registry_and_executor(
    ...
    monkeypatch.setattr(experiment.runtime, "WorkerProcessRegistry", lambda: registry)
    shutdown_calls = _install_interrupting_executor(monkeypatch)
    records.write_immutable_json_exclusive(tmp_path / experiment.RUNTIME_RECEIPT_FILE, {})
    monkeypatch.setattr(experiment, "_validate_actual_runtime_receipt", lambda _value, *, role: {})
...
def test_control_validation_requires_complete_semantics_and_rejects_self_digest_forgery(tmp_path: Path) -> None:
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="semantics differ"):
        experiment._validate_controls(  # noqa: SLF001
            tmp_path,
            config_sha256="7" * 64,
            protocol_sha256=protocol_sha256,
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="declared predicates differ"):
        experiment._validate_controls(  # noqa: SLF001
            forged_root,
            config_sha256=config_sha256,
            protocol_sha256=protocol_sha256,
...
def test_pre_score_projection_rejects_preexisting_later_outputs(
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="wholly absent"):
...
def test_saved_pre_score_budget_rejects_snapshot_mutation(tmp_path: Path) -> None:
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="budget differs"):
        experiment._validate_pre_score_budget(  # noqa: SLF001
            tmp_path,
            protocol_sha256=protocol_sha256,
        )
...
def test_bounded_canonical_validator_rejects_oversize_and_padded_score_json(
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="file bound"):
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="canonical JSON"):
        experiment._validate_bounded_canonical_file(  # noqa: SLF001
            padded,
            maximum_bytes=100,
            label="row score",
...
def test_future_isolation_validation_recomputes_all_manifests_and_exact_grammar(
    ...
    for index, mutation in enumerate(mutations):
        ...
        with pytest.raises(experiment.InvalidMM011Package, match="future-isolation"):
...
@pytest.mark.parametrize(
    ...
    (
        (True, "MM011_GO", "preregister_new_TAESD_MM001_successor_then_audit_before_execution"),
        (
            False,
            "tested_causal_operator_failure_supported",
            "preregister_MM010_source_only_analog_coverage_then_audit_before_execution",
        ),
        (False, "MM011_inconclusive", "neither_branch_diagnose_and_preregister_new_assay"),
        (False, "invalid_MM011", "neither_branch_diagnose_and_preregister_new_assay"),
    ),
)
...
def test_artifact_manifest_is_self_excluding_stable_and_rejects_nonregular_entries(tmp_path: Path) -> None:
    ...
    with pytest.raises(experiment.InvalidMM011Package, match="non-regular entry"):
