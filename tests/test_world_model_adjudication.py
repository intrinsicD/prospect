...
class Harness:
    ...
    def review(self, *, disposition: str) -> Path:
        ...
        return_path = self.tmp_path / "artifacts" / "wm001-reviews" / "formal-v1.13.0.json"
...
def harness(
    ...
    audit_attempt = tmp_path / "operator" / "audits" / "formal-audit-v1.13.0"
    audit_claim = tmp_path / "formal" / "formal-audit-v1.13.0.json"
    package = tmp_path / "adjudication" / "formal-adjudication-v1.13.0"
    adjudication_marker = tmp_path / "formal" / "formal-adjudication-v1.13.0.json"
    outer_root = tmp_path / "outer-completions" / "v1.13"
    value = Harness(
        tmp_path=tmp_path,
    ...
    monkeypatch.setattr(
        ...
        tmp_path / "artifacts" / "wm001-reviews" / "formal-v1.13.0.json",
    )
...
def test_launch_v2_authenticates_real_outer_finalized_binding_attempt(
    ...
    binding_root = lifecycle / "results" / "operator-v1.13" / "bindings"
    binding_attempt = binding_root / "formal-binding-v1.13.0"
    ...
    with pytest.raises(
        AdjudicationError,
        match="unique v1.13 launch record",
    ):
...
def test_preclaim_staging_mutation_does_not_consume_or_replay(
    ...
    assert list(harness.package.parent.glob(".formal-adjudication-v1.13.0.staging-*")) == []
...
def test_marker_only_explicit_recovery_is_zero_replay_and_single_use(
    ...
    holders = list(harness.package.parent.glob(".formal-adjudication-v1.13.0.staging-*/" + ADJUDICATION_CLAIM_NAME))
...
def test_older_marker_does_not_consume_v113(harness: Harness) -> None:
    harness.make_attempt()
    review = harness.review(disposition="accepted")
    harness.install_replay()
