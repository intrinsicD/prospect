...
def test_only_no_override_development_run_can_occupy_qualification_path(
    ...
    for reserved in (
        qualification,
        root / "development-closure-v1.17.0.json",
        root / "v1.17.0" / "preformal",
        tmp_path / "operator-v1.17" / "bindings" / "formal-binding-v1.17.0",
    ):
        with pytest.raises(
            ValueError,
    ...
    assert run._development_output(
        ...
        seed_override=True,
        diagnostic_stamp="stamp",
    ) == (root / "diagnostics-v1.17.0" / "diagnostic-stamp")
